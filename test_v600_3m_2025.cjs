...
// --- Config (v6.0.0 3m interval, 2025 Start) ---
const SYMBOL = 'BTCUSDT';
const FETCH_START_TIME = new Date('2024-11-01T00:00:00+09:00').getTime();
const ACTUAL_START_TIME = new Date('2025-01-01T00:00:00+09:00').getTime();
const ACTUAL_END_TIME = Date.now();
...
async function runV600_3m_2025() {
  console.log(`\n--- [v6.0.0] 3분봉 백테스트 (2025-01-01 ~ 현재) ---`);

  const klines1m_all = await fetchKlines(SYMBOL, '1m', FETCH_START_TIME, ACTUAL_END_TIME);
  const klines3m_all = await fetchKlines(SYMBOL, '3m', FETCH_START_TIME, ACTUAL_END_TIME);
  const klines1h_all = await fetchKlines(SYMBOL, '1h', FETCH_START_TIME, ACTUAL_END_TIME);
  const klines1d_all = await fetchKlines(SYMBOL, '1d', FETCH_START_TIME, ACTUAL_END_TIME);

  const indicators3m = { macd: calculateMACD(klines3m_all.map(k => k.close)), stoch: calculateStochRSI(calculateRSI(klines3m_all.map(k => k.close))) };
  ...
  const indicators1d = { macd: calculateMACD(klines1d_all.map(k => k.close)) }; 
  ...
  const output = ["Time_KST,SignalType,Entry_Price,Entry_Type,Exit_Time,Exit_Price,Result,ROI,Balance,3m_K,3m_D,1h_MACD,1h_Sig,1h_K,1h_D,1d_MACD,1d_Sig,3m_Vol,1h_Vol"];
  let m3Idx = 0;
  ...
  for (let i = 1; i < klines1m.length; i++) {
    ...
    while (m3Idx < klines3m_all.length && klines3m_all[m3Idx].time < time) m3Idx++;
    const idx3m = m3Idx - 1;
    const r1h = klines1h_all.findIndex(k => k.time > time - 3600000) - 1;
    const r1d = klines1d_all.findIndex(k => k.time > time - 86400000) - 1;

    if (idx3m < 0 || r1h < 0 || r1d < 0) continue;

    const k3 = indicators3m.stoch.k[idx3m], d3 = indicators3m.stoch.d[idx3m];
    const m1h = indicators1h.macd.m[r1h], s1h = indicators1h.macd.s[r1h], kh = indicators1h.stoch.k[r1h], dh = indicators1h.stoch.d[r1h];
    const m1d = indicators1d.macd.m[r1d], s1d = indicators1d.macd.s[r1d];

    const cond3m = (k3 > d3) ? 'long' : (k3 < d3 ? 'short' : 'hold');
    const cond1h = (m1h > s1h && kh > dh) ? 'long' : (m1h < s1h && kh < dh ? 'short' : 'hold');
    const cond1d = (m1d > s1d) ? 'long' : (m1d < s1d ? 'short' : 'hold');

    let sig = 'hold';
    if (cond3m === 'long' && cond1h === 'long' && cond1d === 'long') sig = 'long';
    if (cond3m === 'short' && cond1h === 'short' && cond1d === 'short') sig = 'short';

    if (sig !== 'hold' && sig !== currentPosSignal) {
      const marketPriceAtSignal = k1m.close;
      const limitTargetPrice = sig === 'long' ? klines3m_all[idx3m].low : klines3m_all[idx3m].high;
      const vol3m = klines3m_all[idx3m].volume;
      ...
      if (executed) {
        ...
        for (let j = entryTimeIdx; j < klines1m.length; j++) {
          ...
          if (sig === 'long') {
            if (ex.high >= tpPrice) { balance *= (1 + TARGET_NET_ROI); wins++; output.push(`${entryTimeKST},LONG,${finalEntryPrice},${entryType},${toKSTString(ex.time)},${tpPrice.toFixed(2)},WIN,3.00%,${balance.toFixed(2)},${k3},${d3},${m1h},${s1h},${kh},${dh},${m1d},${s1d},${vol3m},${vol1h}`); i = j; break; }
            if (ex.low <= slPrice) { balance *= (1 - (SL_ROI + (MAKER_FEE_RATE + TAKER_FEE_RATE) * LEVERAGE)); losses++; output.push(`${entryTimeKST},LONG,${finalEntryPrice},${entryType},${toKSTString(ex.time)},${slPrice.toFixed(2)},LOSS,-15.00%,${balance.toFixed(2)},${k3},${d3},${m1h},${s1h},${kh},${dh},${m1d},${s1d},${vol3m},${vol1h}`); i = j; break; }
          } else {
            if (ex.low <= tpPrice) { balance *= (1 + TARGET_NET_ROI); wins++; output.push(`${entryTimeKST},SHORT,${finalEntryPrice},${entryType},${toKSTString(ex.time)},${tpPrice.toFixed(2)},WIN,3.00%,${balance.toFixed(2)},${k3},${d3},${m1h},${s1h},${kh},${dh},${m1d},${s1d},${vol3m},${vol1h}`); i = j; break; }
            if (ex.high >= slPrice) { balance *= (1 - (SL_ROI + (MAKER_FEE_RATE + TAKER_FEE_RATE) * LEVERAGE)); losses++; output.push(`${entryTimeKST},SHORT,${finalEntryPrice},${entryType},${toKSTString(ex.time)},${slPrice.toFixed(2)},LOSS,-15.00%,${balance.toFixed(2)},${k3},${d3},${m1h},${s1h},${kh},${dh},${m1d},${s1d},${vol3m},${vol1h}`); i = j; break; }
          }
        }
  ...
  fs.writeFileSync('C:/dev/2026_candy/backtest_v600_3m_2025_results.csv', output.join('\n'));
  console.log(`\n--- 2025 Results (v6.0.0 - 3m Interval) ---`);
  console.log(`ROI: ${((balance/INITIAL_BALANCE - 1)*100).toFixed(1)}%`);
