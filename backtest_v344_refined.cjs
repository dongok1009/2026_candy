...
const MAKER_FEE_RATE = 0.0002; // Limit or better-than-limit market entry
const TAKER_FEE_RATE = 0.0005; // SL
const EXIT_MAKER_FEE_RATE = 0.0002; // TP
...
async function runV344Refined() {
  console.log(`--- [v3.4.4] 백테스트 시작: 에너지필터 제거 + Better Entry + 거래량 로깅 ---`);
      ...
      if (sig === 'long' && marketPriceAtSignal <= limitTargetPrice) {
          ...
          for (let j = i; j < klines1m.length; j++) {
            ...
            if (sig === 'long' && ex.low <= limitTargetPrice) { 
                finalEntryPrice = limitTargetPrice; executed = true; entryTimeIdx = j; entryType = "LIMIT"; break; 
            }
        ...
        for (let j = entryTimeIdx; j < klines1m.length; j++) {
          ...
          if (sig === 'long') {
            if (ex.high >= tpPrice) { 
                ...
                output.push(`${entryTimeKST},LONG,${finalEntryPrice},${entryType},${toKSTString(ex.time)},${tpPrice.toFixed(2)},WIN,3.00%,${balance.toFixed(2)},${k5},${d5},${m1h},${s1h},${kh},${dh},${m1d},${s1d},${vol5m},${vol1h}`); 
                i = j; break; 
            }
            if (ex.low <= slPrice) { 
                balance *= (1 - (SL_ROI + (MAKER_FEE_RATE + TAKER_FEE_RATE) * LEVERAGE)); losses++; 
                output.push(`${entryTimeKST},LONG,${finalEntryPrice},${entryType},${toKSTString(ex.time)},${slPrice.toFixed(2)},LOSS,-15.00%,${balance.toFixed(2)},${k5},${d5},${m1h},${s1h},${kh},${dh},${m1d},${s1d},${vol5m},${vol1h}`); 
                i = j; break; 
            }
          } else {
            if (ex.low <= tpPrice) { 
                balance *= (1 + TARGET_NET_ROI); wins++; 
                output.push(`${entryTimeKST},SHORT,${finalEntryPrice},${entryType},${toKSTString(ex.time)},${tpPrice.toFixed(2)},WIN,3.00%,${balance.toFixed(2)},${k5},${d5},${m1h},${s1h},${kh},${dh},${m1d},${s1d},${vol5m},${vol1h}`); 
                i = j; break; 
            }
            if (ex.high >= slPrice) { 
                balance *= (1 - (SL_ROI + (MAKER_FEE_RATE + TAKER_FEE_RATE) * LEVERAGE)); losses++; 
                output.push(`${entryTimeKST},SHORT,${finalEntryPrice},${entryType},${toKSTString(ex.time)},${slPrice.toFixed(2)},LOSS,-15.00%,${balance.toFixed(2)},${k5},${d5},${m1h},${s1h},${kh},${dh},${m1d},${s1d},${vol5m},${vol1h}`); 
                i = j; break; 
            }
          }
        }
  ...
  console.log(`승률: ${((wins/(wins+losses))*100).toFixed(1)}% (승:${wins}/패:${losses})`);
  fs.writeFileSync('c:/dev/2026_candy/backtest_v344_refined_results.csv', output.join('\n'));
  console.log(`파일 저장 완료: backtest_v344_refined_results.csv`);
}
