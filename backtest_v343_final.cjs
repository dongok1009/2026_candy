...
async function runV343Limit10m() {
  console.log(`--- [v3.4.3] 백테스트: 지정가 10분봉 Low/High 진입 ---`);
  ...
  const output = ["EntryTime,Type,MarketPriceAtSignal,LimitEntryPrice,ExitTime,ExitPrice,Result,ROI,FinalBalance,Status"];
  let m5Idx = 0;

  for (let i = 11; i < klines1m_all.length; i++) {
    const k1m = klines1m_all[i];
    if (k1m.time < ACTUAL_START_TIME) continue;
    ...
    if (sig !== 'hold' && sig !== currentPosSignal) {
      ...
      // Calculate 10m Min/Max using previous 10 1m klines
      const last10mKlines = klines1m_all.slice(i - 10, i);
      const limitEntryPrice = sig === 'long' ? Math.min(...last10mKlines.map(k => k.low)) : Math.max(...last10mKlines.map(k => k.high));
      
      let executed = false;
      let exitIdx = i;

      for (let j = i; j < klines1m_all.length; j++) {
        const ex = klines1m_all[j];
      ...
      if (executed) {
        let entryTimeKST = toKSTString(klines1m_all[exitIdx].time);
        ...
        for (let j = exitIdx; j < klines1m_all.length; j++) {
          const ex = klines1m_all[j];
          if (new Date(ex.time).getUTCMinutes() === 0 && [0, 8, 16].includes(new Date(ex.time).getUTCHours())) balance *= (1 - (FUNDING_FEE_RATE * LEVERAGE));

          if (sig === 'long') {
            if (ex.high >= tpPrice) { balance *= (1 + TARGET_NET_ROI); wins++; output.push(`${entryTimeKST},LONG,${marketPriceAtSignal},${limitEntryPrice},${toKSTString(ex.time)},${tpPrice.toFixed(2)},WIN,3.00%,${balance.toFixed(2)},Executed`); i = j; break; }
            if (ex.low <= slPrice) { balance *= (1 - (SL_ROI + (MAKER_FEE_RATE + TAKER_FEE_RATE) * LEVERAGE)); losses++; output.push(`${entryTimeKST},LONG,${marketPriceAtSignal},${limitEntryPrice},${toKSTString(ex.time)},${slPrice.toFixed(2)},LOSS,-15.00%,${balance.toFixed(2)},Executed`); i = j; break; }
          } else {
            if (ex.low <= tpPrice) { balance *= (1 + TARGET_NET_ROI); wins++; output.push(`${entryTimeKST},SHORT,${marketPriceAtSignal},${limitEntryPrice},${toKSTString(ex.time)},${tpPrice.toFixed(2)},WIN,3.00%,${balance.toFixed(2)},Executed`); i = j; break; }
            if (ex.high >= slPrice) { balance *= (1 - (SL_ROI + (MAKER_FEE_RATE + TAKER_FEE_RATE) * LEVERAGE)); losses++; output.push(`${entryTimeKST},SHORT,${marketPriceAtSignal},${limitEntryPrice},${toKSTString(ex.time)},${slPrice.toFixed(2)},LOSS,-15.00%,${balance.toFixed(2)},Executed`); i = j; break; }
          }
        }
  ...
  fs.writeFileSync('C:/dev/2026_candy/backtest_v343_results.csv', output.join('\n'));
}
