...
async function run() {
  ...
  const LEV = 5, TP_NET = 0.05, SL_NET = 0.12; // 5% TP, 12% SL Optimized!
  const SLIPPAGE = 0.0005, FEE_MARGIN = (0.0002+0.0002+SLIPPAGE)*LEV;

  let wins = 0, losses = 0;
  for(let i=0; i<k1m.length; i++){
    const k = k1m[i];
    while(m5Idx < k5m.length && k5m[m5Idx].time <= k.time - 300000) m5Idx++;
    let r5 = m5Idx-1, r1h = -1;
    ...
    if(!pos && gSig !== 'HOLD' && gSig !== currentGSignal) {
      const entry = k.open; 
      if(gSig==='LONG') pos = { side:'LONG', entryPrice: entry, tp: entry*(1+(TP_NET+FEE_MARGIN)/LEV), sl: entry*(1-SL_NET/LEV)};
      else pos = { side:'SHORT', entryPrice: entry, tp: entry*(1-(TP_NET+FEE_MARGIN)/LEV), sl: entry*(1+SL_NET/LEV)};
    }
    currentGSignal = gSig;
  ...
  console.log(`\n=== Optimized Market v6.0.0 Report (5% TP / 12% SL) ===`);
  console.log(`Final Balance: $${balance.toFixed(2)}`);
  console.log(`ROI: ${((balance-1000)/1000*100).toFixed(2)}%`);
