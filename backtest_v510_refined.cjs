...
async function runV510Refined() {
  console.log(`--- [v5.1.0] 백테스트: 지능형 진입가 (Market vs Limit 최선책) ---`);
  ...
  const output = ["EntryTime,Type,SignalPrice,LimitPrice,EntryPrice,ExitTime,ExitPrice,Result,ROI,FinalBalance"];
  let m5Idx = 0;
  ...
  for (let i = 1; i < klines1m.length; i++) {
    ...
    if (sig !== 'hold' && sig !== currentPosSignal) {
      const signalPrice = k1m.open; // Signal is confirmed at end of idx5m, open of k1m is the "current price"
      const prevLow = klines5m_all[idx5m].low;
      const prevHigh = klines5m_all[idx5m].high;
      
      // v5.1.0 Logic: Market vs Limit (Whichever is better)
      const targetEntryPrice = sig === 'long' ? Math.min(signalPrice, prevLow) : Math.max(signalPrice, prevHigh);
      
      let executed = false;
      let finalEntryPrice = targetEntryPrice;
      let exitIdx = i;

      // Immediate match?
      if (sig === 'long' && k1m.low <= targetEntryPrice) { executed = true; }
      else if (sig === 'short' && k1m.high >= targetEntryPrice) { executed = true; }
      else {
        // Search in next bars
        for (let j = i + 1; j < klines1m.length; j++) {
            const ex = klines1m[j];
            if (sig === 'long' && ex.low <= targetEntryPrice) { executed = true; exitIdx = j; break; }
            if (sig === 'short' && ex.high >= targetEntryPrice) { executed = true; exitIdx = j; break; }
            if (j - i > 60) break; // Timeout (1 hour)
        }
      }
      ...
      if (executed) {
        ...
        for (let j = exitIdx; j < klines1m.length; j++) {
          ...
          if (sig === 'long') {
            if (ex.high >= tpPrice) { balance *= (1 + TARGET_NET_ROI); wins++; output.push(`${entryTimeKST},LONG,${signalPrice},${prevLow},${finalEntryPrice},${toKSTString(ex.time)},${tpPrice.toFixed(2)},WIN,3.00%,${balance.toFixed(2)}`); i = j; break; }
            if (ex.low <= slPrice) { balance *= (1 - (SL_ROI + (MAKER_FEE_RATE + TAKER_FEE_RATE) * LEVERAGE)); losses++; output.push(`${entryTimeKST},LONG,${signalPrice},${prevLow},${finalEntryPrice},${toKSTString(ex.time)},${slPrice.toFixed(2)},LOSS,-15.00%,${balance.toFixed(2)}`); i = j; break; }
          } else {
            if (ex.low <= tpPrice) { balance *= (1 + TARGET_NET_ROI); wins++; output.push(`${entryTimeKST},SHORT,${signalPrice},${prevHigh},${finalEntryPrice},${toKSTString(ex.time)},${tpPrice.toFixed(2)},WIN,3.00%,${balance.toFixed(2)}`); i = j; break; }
            if (ex.high >= slPrice) { balance *= (1 - (SL_ROI + (MAKER_FEE_RATE + TAKER_FEE_RATE) * LEVERAGE)); losses++; output.push(`${entryTimeKST},SHORT,${signalPrice},${prevHigh},${finalEntryPrice},${toKSTString(ex.time)},${slPrice.toFixed(2)},LOSS,-15.00%,${balance.toFixed(2)}`); i = j; break; }
          }
        }
  ...
  fs.writeFileSync('C:/dev/2026_candy/backtest_v510_results.csv', output.join('\n'));
}
