// v8.2.7 매매 전략 로직 모듈 (v8.2.6 + RCI 이중(9/26) 교차 필터, TRIX(14) 시그널선 교차 필터)
const { calculateEMA, calculateSMA, calculateRSI, calculateMACD, calculateStochRSI, calculateADX, calculateBBW, calculateBBWP, calculateROC, calculateSlope, calculateRCI, calculateTRIX } = require('../lib/indicators.cjs');

console.log(`[LOADED] Logic.v8.2.7.cjs loaded at ${new Date().toISOString()}`);

// 룰에 등장하는 TRIX 시그널 기간을 모아 반환 (기본 9 포함).
// 봉마다 EMA를 다시 도는 대신 필요한 기간별 시그널선을 미리 계산해 두기 위함.
const collectTrixSignalPeriods = (overrideRules) => {
    const set = new Set([9]);
    if (overrideRules) {
        ['long', 'short'].forEach(side => {
            ['5m', '1h', '1d'].forEach(tf => {
                const r = overrideRules[side]?.[tf];
                const sp = r && r.trixSignalPeriod !== undefined ? parseInt(r.trixSignalPeriod) : null;
                if (sp && sp > 0) set.add(sp);
            });
        });
    }
    return set;
...
const strategy = {
    ...
    indicators_logic: (klines, overrideRules) => {
        ...
        const buildRciTrix = (closes) => {
            ...
            trixSignalPeriods.forEach(sp => {
                ...
                out[`trix_sig_${sp}`] = r.signal;
            });
            out.trix_sig = out.trix_sig_9;                  // 로그·차트 기본 시그널선
            return out;
        };
        const m5RciTrix = buildRciTrix(m5Closes);
        const h1RciTrix = buildRciTrix(h1Closes);
        const d1RciTrix = buildRciTrix(d1Closes);
    ...
    signal_logic: (indicators, indices, overrideRules) => {
        ...
        const checkCondition = (side, interval, idx, indicatorsObj, idx1mVal) => {
            ...
            // 11. TRIX Cross Filter (시그널선 교차: 롱 TRIX > Signal, 숏 TRIX < Signal / 시그널 기간 룰별 가변)
            if ((chk('trixCrossEnabled') || chk('useTrixCross')) && data.trix) {
                const sp = rules && rules.trixSignalPeriod !== undefined ? parseInt(rules.trixSignalPeriod) : 9;
                const sigArr = data[`trix_sig_${sp}`] || data.trix_sig;
                const tx = data.trix[idx];
                const sg = sigArr ? sigArr[idx] : null;
                logDetail += `TRIX(sig${sp}):${tx !== null && tx !== undefined ? tx.toFixed(4) : 'null'}/${sg !== null && sg !== undefined ? sg.toFixed(4) : 'null'} `;
                if (tx !== null && tx !== undefined && sg !== null && sg !== undefined) {
                    if (side === 'long' && tx <= sg) match = false;
                    if (side === 'short' && tx >= sg) match = false;
                }
            }
