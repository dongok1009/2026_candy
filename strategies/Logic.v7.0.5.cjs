...
console.log(`[LOADED] Logic.v7.0.5.cjs loaded at ${new Date().toISOString()}`);

const strategy = {
    name: 'Logic.v7.0.5',
    description: 'v7.0.5 (3m Entry Edition)',
    header: "Entry_Time,Exit_Time,Balance,Cum_ROI,Side,Entry_Price,Exit_Price,Net_Profit,ROE,Quantity,Fee,FundingFee,M3_StochK,M3_StochD,M3_ADX,H1_MACD,H1_Sig,H1_StochK,H1_StochD,H1_ADX,D1_MACD,D1_Sig,D1_StochK,D1_StochD,D1_ADX",
    ...
    indicators_logic: (klines) => {
        return {
            m3: {
                macd: calculateMACD(klines.m3.map(k => k.close)),
                stoch: calculateStochRSI(calculateRSI(klines.m3.map(k => k.close))),
                adx: calculateADX(klines.m3)
            },
            h1: {
    ...
    signal_logic: (indicators, indices, overrideRules) => {
        ...
        const checkCondition = (side, interval, idx, indicatorsObj) => {
            ...
            if (!rules) {
                if (side === 'long') {
                    if (interval === 'm3') return data.adx[idx] >= 30 && data.stoch.k[idx] > data.stoch.d[idx];
        ...
        const longMatch = ['m3', 'h1', 'd1'].every(iv => {
            const idx = iv === 'm3' ? idx3m : (iv === 'h1' ? r1h : r1d);
            return checkCondition('long', iv, idx, indicators);
        });

        const shortMatch = ['m3', 'h1', 'd1'].every(iv => {
            const idx = iv === 'm3' ? idx3m : (iv === 'h1' ? r1h : r1d);
    ...
    entry_logic: (sig, k1m, k3_prev, klines1m, currentIndex, config) => {
        const signalPrice = k1m.open;
        const targetPrice = sig === 'long' ? k3_prev.low : k3_prev.high;
