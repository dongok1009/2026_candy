...
console.log(`[LOADED] Logic.v8.2.0.cjs loaded at ${new Date().toISOString()}`);

const strategy = {
    name: 'Logic.v8.2.0',
    description: 'v8.2.0 (RSI AND Condition & CSV Reporting Support)',
    ...
    signal_logic: (indicators, indices, overrideRules) => {
        ...
        const checkCondition = (side, interval, idx, indicatorsObj) => {
            ...
            // [SIDE-FIRST NESTING] Unified structure: rules[side][uiInterval]
            let rules = null;
            if (overrideRules) {
