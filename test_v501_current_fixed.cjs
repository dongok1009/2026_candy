...
const calculateMACD = (closes, fast = 12, slow = 26, signal = 9) => {
  ...
  const sEMA = calculateMACD_Inner_EMA(closes, slow); 
  // Wait, I should use the same code as before.
  const sEMA2 = calculateEMA(closes, slow);
  const m = fEMA.map((f, i) => (f !== null && sEMA2[i] !== null) ? f - sEMA2[i] : null);
...
function calculateMACD_Inner_EMA(data, p) { return calculateEMA(data, p); }
...
const ACTUAL_END_TIME = new Date('2026-03-31T23:59:59+09:00').getTime();
...
async function runV501CurrentFixed() {
  console.log(`--- [v5.0.1] 2025-2026 백테스트 (필터 수정 완료: 1d StochRSI 추가) ---`);
  ...
  fs.writeFileSync('C:/dev/2026_candy/backtest_v501_20250101_20260331_results_fixed.csv', output.join('\n'));
  console.log(`\n--- 2025-2026 Results (Fixed Filter) ---`);
