...
async function run() {
  ...
  const out = ["Time_KST,1m_Open,1m_High,1m_Low,1m_Close,5m_K,5m_D,1h_MACD,1h_Sig,1h_K,1h_D,1d_MACD,1d_Sig,GlobalSignal,PositionSide,EntryPrice,ExitPrice,ROI,Balance,LimitSource,Comment"];
  ...
  for(let i=0; i<k1m.length; i++){
    ...
    let rowComment = "", rowLimitSrc = "";

    // 1. Order Waiting Logic
    if(pendingOrder && !pos) {
      if(pendingOrder.side === 'LONG' && k.low <= pendingOrder.limitPrice) {
        ...
        rowComment = `Filled at ${actualEntry.toFixed(2)}`;
        rowLimitSrc = pendingOrder.sourceTime;
        pendingOrder = null;
    ...
    if(!pos && !pendingOrder && gSig !== 'HOLD' && gSig !== currentGSignal) {
      ...
      pendingOrder = { side: gSig, limitPrice: limit, waitingMinutes: 0, sourceTime: `5m OHLC of ${toKST(k5m[r5].time)}` };
      rowComment = `Order Placed at ${limit.toFixed(2)}`;
      rowLimitSrc = pendingOrder.sourceTime;
    }
    currentGSignal = gSig;

    // Debugging specific point for 2025-01-03 17:15
    if(k.time >= 1735892100000 && k.time <= 1735892500000) { // 17:15 +/- 3min
       // console.log(`[DEBUG CP] Time: ${toKST(k.time)} | k5m[r5]: ${toKST(k5m[r5].time)} High: ${k5m[r5].high}`); 
    }
    ...
    out.push(`${toKST(k.time)},${k.open},${k.high},${k.low},${k.close},${k5.toFixed(2)},${d5.toFixed(2)},${m1h.toFixed(2)},${s1h.toFixed(2)},${kh.toFixed(2)},${dh.toFixed(2)},${m1d.toFixed(2)},${s1d.toFixed(2)},${gSig},${pos?pos.side:""},${pos?pos.entryPrice.toFixed(2):""},${rowExit},${rowROI},${balance.toFixed(2)},"${rowLimitSrc}","${rowComment}"`);
  }
  fs.writeFileSync('v600_1m_detail_STRICT_V3_250101_250301.csv', out.join('\n'));
  console.log("Strict V3 Export Complete: v600_1m_detail_STRICT_V3_250101_250301.csv");
}
run();
