...
const START_TIME = new Date('2025-09-01T00:00:00+09:00').getTime(); 
const ACTUAL_START_TIME = new Date('2026-01-01T00:00:00+09:00').getTime();
const END_TIME = new Date('2026-03-25T00:00:00+09:00').getTime();
...
async function runOptimization() {
  ...
  const searchTP = [0.03, 0.04, 0.05];
  const searchSL = [0.10, 0.15, 0.20];
  const searchHist = [200, 300, 400];
  const searchStoch = [true, false];
  const search5mMACDValue = [true, false]; // MACD < -10 filter

  const results = [];

  console.log("Deep Tuning... " + (searchTP.length * searchSL.length * searchHist.length * searchStoch.length * search5mMACDValue.length) + " combinations.");

  for (let tpVal of searchTP) {
    for (let slVal of searchSL) {
      for (let histThreshold of searchHist) {
        for (let useStoch of searchStoch) {
          for (let use5mVal of search5mMACDValue) {
            ...
            const totalFees = (TAKER_FEE_RATE + MAKER_FEE_RATE) * LEVERAGE;
            const grossTP = tpVal + totalFees;
            const grossSL = slVal + totalFees;
            ...
            for (let i = 1; i < klines5m.length; i++) {
              ...
              if (global !== prevSignal && global !== 'hold') {
                ...
                const slPrice = global === 'long' ? entryPrice * (1 - (grossSL/LEVERAGE)) : entryPrice * (1 + (grossSL/LEVERAGE));
            ...
            results.push({ tp: tpVal, sl: slVal, hist: histThreshold, stoch: useStoch, val5m: use5mVal, balance, winRate: (wins/(wins+losses)) });
          }
        }
      }
    }
  ...
  console.log("BEST 10 COMBINATIONS (NO OVERLAP):");
  results.slice(0, 10).forEach((r, idx) => {
    console.log(`${idx+1}. TP: ${(r.tp*100).toFixed(0)}%, SL: ${(r.sl*100).toFixed(0)}%, Hist: ${r.hist}, Stoch: ${r.stoch ? 'ON' : 'OFF'}, 5mVal: ${r.val5m ? 'ON' : 'OFF'}, WR: ${(r.winRate*100).toFixed(1)}%, Balance: $${r.balance.toFixed(2)}`);
  });
}
