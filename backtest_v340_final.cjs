...
async function runV340LimitEntry() {
  console.log(`--- [v3.4.0] 백테스트: 지정가 진입 로직 ---`);
  ...
  for (let i = 1; i < klines1m.length; i++) {
    ...
    if (sig !== 'hold' && sig !== currentPosSignal) {
      // LIMIT ENTRY START (v3.4.0: Using Prev 5m Low/High)
      ...
      for (let j = i; j < klines1m.length; j++) {
        ...
        if (ex.time % 300000 === 0) { // On 5m close
  ...
  fs.writeFileSync('C:/dev/2026_candy/backtest_v340_results.csv', output.join('\n'));
}
