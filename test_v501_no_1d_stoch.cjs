...
// --- Config (v5.0.1 without 1d StochRSI) ---
...
const ACTUAL_END_TIME = new Date('2026-03-30T23:59:59+09:00').getTime();
...
async function runV501No1dStoch() {
  console.log(`--- [v5.0.1 실험] 1d StochRSI 조건 제거 백테스트 ---`);
  ...
  const indicators1d = { macd: calculateMACD(klines1d_all.map(k => k.close)) }; // 1d Stoch omitted
  ...
  for (let i = 1; i < klines1m.length; i++) {
    ...
    let sig = 'hold';
    if (cond5m === 'long' && cond1h === 'long' && m1d > s1d) sig = 'long';
    if (cond5m === 'short' && cond1h === 'short' && m1d < s1d) sig = 'short';
  ...
  fs.writeFileSync('C:/dev/2026_candy/backtest_v501_no_1d_stoch_results.csv', output.join('\n'));
  console.log(`\n--- 실험 결과 (1d StochRSI 제거) ---`);
