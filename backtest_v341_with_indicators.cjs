...
async function runV341WithIndicators() {
  console.log(`--- [v3.4.1] 백테스트: 지정가 진입 (10배, 6%/30%) + 지표 포함 CSV 생성 ---`);
  ...
  const header = "EntryTime,Type,EntryPrice,ExitTime,Result,ROI,FinalBalance,Status,5m_Stoch_K,5m_Stoch_D,1h_MACD,1h_MACD_Sig,1h_Stoch_K,1h_Stoch_D,1d_MACD,1d_MACD_Sig,1d_Stoch_K,1d_Stoch_D,1d_Energy";
  const output = [header];
  let m5Idx = 0;
  ...
  for (let i = 1; i < klines1m.length; i++) {
    ...
    if (sig !== 'hold' && sig !== currentPosSignal) {
      ...
      if (executed) {
        ...
        const indicatorString = `${k5},${d5},${m1h},${s1h},${kh},${dh},${m1d},${s1d},${kd},${dd},${hd}`;
        ...
        for (let j = exitIdx; j < klines1m.length; j++) {
          ...
          if (sig === 'long') {
            if (ex.high >= tpPrice) { 
                ...
                output.push(`${entryTimeKST},LONG,${limitEntryPrice},${toKSTString(ex.time)},WIN,6.00%,${balance.toFixed(2)},Executed,${indicatorString}`); 
                i = j; break; 
            }
            if (ex.low <= slPrice) { 
                balance *= (1 - (SL_ROI + (MAKER_FEE_RATE + TAKER_FEE_RATE) * LEVERAGE)); losses++; 
                output.push(`${entryTimeKST},LONG,${limitEntryPrice},${toKSTString(ex.time)},LOSS,-30.00%,${balance.toFixed(2)},Executed,${indicatorString}`); 
                i = j; break; 
            }
          } else {
            if (ex.low <= tpPrice) { 
                balance *= (1 + TARGET_NET_ROI); wins++; 
                output.push(`${entryTimeKST},SHORT,${limitEntryPrice},${toKSTString(ex.time)},WIN,6.00%,${balance.toFixed(2)},Executed,${indicatorString}`); 
                i = j; break; 
            }
            if (ex.high >= slPrice) { 
                balance *= (1 - (SL_ROI + (MAKER_FEE_RATE + TAKER_FEE_RATE) * LEVERAGE)); losses++; 
                output.push(`${entryTimeKST},SHORT,${limitEntryPrice},${toKSTString(ex.time)},LOSS,-30.00%,${balance.toFixed(2)},Executed,${indicatorString}`); 
                i = j; break; 
            }
          }
        }
  ...
  fs.writeFileSync('C:/dev/2026_candy/backtest_v341_results_with_indicators.csv', output.join('\n'));
}

runV341WithIndicators();
