...
async function run(retracePercent) {
  ...
  const LEV = 5, TP_NET = 0.030, SL_NET = 0.15, WAIT_MINUTES = 120; // 120분 대정기
  ...
  for(let i=0; i<k1m.length; i++){
    ...
    if(pendingOrder && !pos) {
      if(pendingOrder.side === 'LONG' && k.low <= pendingOrder.limitPrice) {
        ...
        if(pendingOrder.waiting >= WAIT_MINUTES) pendingOrder = null; 
      }
    }

    if(!pos && !pendingOrder && gSig !== 'HOLD' && gSig !== currentGSignal) {
      const signalPrice = k.open; 
      const limit = gSig==='LONG' ? signalPrice * (1 - retracePercent/100) : signalPrice * (1 + retracePercent/100);
    ...
    if(pos) {
      if(pos.side==='LONG'){
        ...
        else if(k.low<=pos.sl){ balance*=(1-(SL_NET+0.0007*LEV)); losses++; pos=null; }
      } else {
        if(k.low<=pos.tp){ balance*=(1+TP_NET); wins++; pos=null; }
        else if(k.high>=pos.sl){ balance*=(1-(SL_NET+0.0007*LEV)); losses++; pos=null; }
      }
    }
  }

  console.log(`\n=== RETRACE 120M TEST: ${retracePercent}% ===`);
  console.log(`Final Balance: $${balance.toFixed(2)}`);
  console.log(`Win Rate: ${(wins/(wins+losses)*100).toFixed(2)}% (${wins}W / ${losses}L)`);
}

const args = process.argv.slice(2);
const p = parseFloat(args[0]) || 0.5;
run(p);
