...
const resFile = `backtest_2025_immediate_min_results.csv`;
...
async function run() {
    ...
    const WAIT_LIMIT = 240; // Default wait limit 4 hours if not specified
    ...
    for (let i = 0; i < k1m.length; i++) {
        ...
        // Entry Logic: Immediate entry on signal price hit (Min/Max based on confirmed candle)
        if (pendingOrder && !pos) {
            if (pendingOrder.side === 'LONG' && k.low <= pendingOrder.limitPrice) {
                ...
                pos = { 
                    side: 'LONG', 
                    entryPrice: entry, 
                    tp: entry * (1 + (TP_NET + FEE_MARGIN) / LEV), 
                ...
                if (pendingOrder.waiting >= WAIT_LIMIT) pendingOrder = null;
            }
        }

        if (!pos && !pendingOrder && gSig !== 'HOLD' && gSig !== currentGSignal) {
            // "확정봉 기준으로 min값 지정가 진입"
            // For LONG: low of the signal candle (k5m[r5].low)
            // For SHORT: high of the signal candle (k5m[r5].high)
        ...
        if (pos) {
            ...
            if (pos.side === 'LONG') {
                if (k.high >= pos.tp) { 
                    p = balance * TP_NET; balance += p; 
                    const duration = (k.time - pos.entryTime) / 60000;
                    trades.push({
                    ...
                    p = balance * (-FIXED_SL - 0.0035); balance += p; 
                    const duration = (k.time - pos.entryTime) / 60000;
                    trades.push({
                        entryTime: toKSTString(pos.entryTime),
                        exitTime: toKSTString(k.time),
    ...
    console.log(`\n✅ 2025 Backtest (Immediate Min Entry) Complete!`);
    console.log(`Final Balance: $${balance.toFixed(2)}`);
    console.log(`ROI: ${((balance - 1000) / 10).toFixed(2)}%`);
    console.log(`Win Rate: ${(wins / trades.length * 100).toFixed(2)}% (${wins}W / ${losses}L)`);
    console.log(`Total Trades: ${trades.length}`);
    console.log(`Report saved to ${resFile}`);
}
run();
