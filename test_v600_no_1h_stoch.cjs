...
// --- Config (v6.0.0 Experiment: No 1h StochRSI) ---
...
async function runV600No1hStoch() {
  console.log(`--- [v6.0.0 실험] 1h StochRSI 조건 제거 백테스트 ---`);
  ...
  const indicators1h = { macd: calculateMACD(klines1h_all.map(k => k.close)) }; // 1h StochRSI omitted
  const indicators1d = { macd: calculateMACD(klines1d_all.map(k => k.close)) }; // 1d StochRSI omitted
  ...
  for (let i = 1; i < klines1m.length; i++) {
    ...
    const m1h = indicators1h.macd.m[r1h], s1h = indicators1h.macd.s[r1h];
    const m1d = indicators1d.macd.m[r1d], s1d = indicators1d.macd.s[r1d];

    const cond5m = (k5 > d5) ? 'long' : (k5 < d5 ? 'short' : 'hold');
    const cond1h = (m1h > s1h) ? 'long' : (m1h < s1h ? 'short' : 'hold');
    ...
    if (sig !== 'hold' && sig !== currentPosSignal) {
      ...
      if (executed) {
        ...
        for (let j = entryTimeIdx; j < klines1m.length; j++) {
          ...
          if (sig === 'long') {
            if (ex.high >= tpPrice) { balance *= (1 + TARGET_NET_ROI); wins++; output.push(`${entryTimeKST},LONG,${finalEntryPrice},${entryType},${toKSTString(ex.time)},${tpPrice.toFixed(2)},WIN,3.00%,${balance.toFixed(2)},${k5},${d5},${m1h},${s1h},0,0,${m1d},${s1d},${vol5m},${vol1h}`); i = j; break; }
            if (ex.low <= slPrice) { balance *= (1 - (SL_ROI + (MAKER_FEE_RATE + TAKER_FEE_RATE) * LEVERAGE)); losses++; output.push(`${entryTimeKST},LONG,${finalEntryPrice},${entryType},${toKSTString(ex.time)},${slPrice.toFixed(2)},LOSS,-15.00%,${balance.toFixed(2)},${k5},${d5},${m1h},${s1h},0,0,${m1d},${s1d},${vol5m},${vol1h}`); i = j; break; }
          } else {
            if (ex.low <= tpPrice) { balance *= (1 + TARGET_NET_ROI); wins++; output.push(`${entryTimeKST},SHORT,${finalEntryPrice},${entryType},${toKSTString(ex.time)},${tpPrice.toFixed(2)},WIN,3.00%,${balance.toFixed(2)},${k5},${d5},${m1h},${s1h},0,0,${m1d},${s1d},${vol5m},${vol1h}`); i = j; break; }
            if (ex.high >= slPrice) { balance *= (1 - (SL_ROI + (MAKER_FEE_RATE + TAKER_FEE_RATE) * LEVERAGE)); losses++; output.push(`${entryTimeKST},SHORT,${finalEntryPrice},${entryType},${toKSTString(ex.time)},${slPrice.toFixed(2)},LOSS,-15.00%,${balance.toFixed(2)},${k5},${d5},${m1h},${s1h},0,0,${m1d},${s1d},${vol5m},${vol1h}`); i = j; break; }
          }
        }
  ...
  fs.writeFileSync('C:/dev/2026_candy/backtest_v600_no_1h_results.csv', output.join('\n'));
  console.log(`\n--- 실험 결과 (1h/1d StochRSI 모두 제거) ---`);
  console.log(`기존 v6.0.0(only 1d stoch off): +2910%`);
  console.log(`최종 잔고: $${balance.toFixed(2)}`);
  console.log(`ROI: ${((balance/INITIAL_BALANCE - 1)*100).toFixed(1)}%`);
  console.log(`승률: ${((wins/(wins+losses))*100).toFixed(1)}% (W:${wins}/L:${losses})`);
}
runV600No1hStoch();
