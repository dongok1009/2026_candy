...
async function run() {
  console.log(`🚀 [2025] Single Parameter Export: Retrace 1.5% / Timeout 3h`);
  const raw = JSON.parse(fs.readFileSync('backtest_cache_2025.json', 'utf8'));
  const k5m = raw.k5m;
  const k1h = raw.k1h;
  const k1d = raw.k1d;

  const i5m = { stoch: calculateStochRSI(calculateRSI(k5m.map(d=>d.close))) };
  const i1h = { macd: calculateMACD(k1h.map(d=>d.close)), stoch: calculateStochRSI(calculateRSI(k1h.map(d=>d.close))) };
  ...
  for(let i = 100; i < k5m.length; i++) {
    ...
    let sig = 'hold';
    if(c5==='long' && ch==='long' && cd==='long') sig = 'long';
    if(c5==='short' && ch==='short' && cd==='short') sig = 'short';

    const p5 = (i5m.stoch.k[i-1] > i5m.stoch.d[i-1]) ? 'long' : ((i5m.stoch.k[i-1] < i5m.stoch.d[i-1]) ? 'short' : 'hold');
    const ph = (i1h.macd.m[hIdx] > i1h.macd.s[hIdx] && i1h.stoch.k[hIdx-1] > i1h.stoch.d[hIdx-1]) ? 'long' : (i1h.macd.m[hIdx] < i1h.macd.s[hIdx] && i1h.stoch.k[hIdx-1] < i1h.stoch.d[hIdx-1] ? 'short' : 'hold');
    const pd = i1d.macd.m[dIdx] > i1d.macd.s[dIdx] ? 'long' : (i1d.macd.m[dIdx] < i1d.macd.s[dIdx] ? 'short' : 'hold');
    let prevSig = (p5==='hold' || ph==='hold' || pd==='hold') ? 'hold' : 'not_hold';
    ...
    if (sig !== 'hold' && prevSig === 'hold') {
      ...
      let found = false;
      for (let k = i + 1; k < i + 36; k++) { // 3 hours = 36 * 5m
        if (!k5m[k]) break;
  ...
  // Export to CSV
  const header = "Time,Type,SignalPrice,EntryPrice,ExitPrice,Result,Profit,Balance\n";
  const rows = trades.map(t => `${t.time},${t.type},${t.signal},${t.entry},${t.exit},${t.result},${t.profit},${t.balance}`).join("\n");
  fs.writeFileSync('backtest_2025_csv_results.csv', header + rows);
  
  console.log(`\n✅ 2025 Backtest Complete!`);
  console.log(`Final Balance: $${balance.toFixed(2)}`);
  console.log(`Total Trades: ${trades.length}`);
  console.log(`Results saved to: backtest_2025_csv_results.csv`);
}
run();
