...
// v3.5.0 Correct Settings (Based on v3.4.0 + Re-entry + 3% TP)
...
async function runV350ReentryCorrect() {
  console.log(`--- [v3.5.0] 백테스트: 지정가 진입(5분봉 저가/고가) + 재진입 허용 ---`);
  console.log(`조건: v3.4.0 지정가 진입(5분봉) + **재진입 허용** + 3% 익절`);
  ...
  for (let i = 1; i < klines1m.length; i++) {
    ...
    if (sig !== 'hold') { // v3.5.0: Re-entry enabled
      // LIMIT ENTRY START (v3.4.0 logic: 5m low/high)
      ...
      for (let j = i; j < klines1m.length; j++) {
        ...
        if (j - i > 60) break; // Timeout after 60 mins
      }
  ...
  fs.writeFileSync('C:/dev/2026_candy/backtest_v350_results.csv', output.join('\n'));
}

runV350ReentryCorrect();
