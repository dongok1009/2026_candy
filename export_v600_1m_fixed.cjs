...
// --- Indicators (Simplified for brevity inside the same file) ---
...
async function run() {
  ...
  const out = ["Time_KST,1m_Open,1m_High,1m_Low,1m_Close,5m_K,5m_D,1h_MACD,1h_Sig,1h_K,1h_D,1d_MACD,1d_Sig,GlobalSignal,PositionSide,EntryPrice,ExitPrice,ROI,Balance"];

  for(let i=0; i<k1m.length; i++){
    const k = k1m[i]; if(k.time < START_TIME) continue;

    while(m5Idx < k5m.length && k5m[m5Idx].time < k.time) m5Idx++;
    ...
    // 1. Order Waiting Logic (Fixing Look-ahead bias)
    if(pendingOrder && !pos) {
      if(pendingOrder.side === 'LONG' && k.low <= pendingOrder.limitPrice) {
        pos = { side:'LONG', entryPrice: pendingOrder.limitPrice, tp: pendingOrder.limitPrice*(1+(TP_NET+FEE_MARGIN)/LEV), sl: pendingOrder.limitPrice*(1-SL_NET/LEV)};
        pendingOrder = null;
      } else if(pendingOrder.side === 'SHORT' && k.high >= pendingOrder.limitPrice) {
        pos = { side:'SHORT', entryPrice: pendingOrder.limitPrice, tp: pendingOrder.limitPrice*(1-(TP_NET+FEE_MARGIN)/LEV), sl: pendingOrder.limitPrice*(1+SL_NET/LEV)};
        pendingOrder = null;
      } else {
        pendingOrder.waitingMinutes++;
        if(pendingOrder.waitingMinutes >= 60) pendingOrder = null; // Timeout
      }
    }
    ...
    out.push(`${toKST(k.time)},${k.open},${k.high},${k.low},${k.close},${k5.toFixed(2)},${d5.toFixed(2)},${m1h.toFixed(2)},${s1h.toFixed(2)},${kh.toFixed(2)},${dh.toFixed(2)},${m1d.toFixed(2)},${s1d.toFixed(2)},${gSig},${pos?pos.side:""},${pos?pos.entryPrice.toFixed(2):""},${rowExit},${rowROI},${balance.toFixed(2)}`);
  }
  fs.writeFileSync('v600_1m_detail_fixed_250101_250301.csv', out.join('\n'));
  console.log("Fixed Export Complete: v600_1m_detail_fixed_250101_250301.csv");
}
run();
