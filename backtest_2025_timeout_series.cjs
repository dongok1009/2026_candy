...
async function runBacktest(timeoutHours) {
    const resFile = `backtest_2025_timeout_${timeoutHours}h.csv`;
    ...
    const FEE_MARGIN = (0.0002 + 0.0002) * LEV; // Maker fee for regular entry/exit
    const TIMEOUT_MS = timeoutHours * 3600000;
        ...
        if (pos) {
            ...
            if (pos.side === 'LONG') {
                if (k.high >= pos.tp) { p = balance * TP_NET; exitReason = 'WIN'; }
                    ...
                    // Force Market Exit
                    const exitP = k.close;
                    const rawProfit = (exitP - pos.entryPrice) / pos.entryPrice * LEV;
                    const netProfit = rawProfit - 0.0007 * LEV; // Entry maker(0.02) + Exit taker(0.05)
                    p = balance * netProfit;
                    exitReason = 'TIMEOUT';
                }
            } else {
                if (k.low <= pos.tp) { p = balance * TP_NET; exitReason = 'WIN'; }
                    ...
                    // Force Market Exit
                    const exitP = k.close;
                    const rawProfit = (pos.entryPrice - exitP) / pos.entryPrice * LEV;
                    const netProfit = rawProfit - 0.0007 * LEV; 
                    p = balance * netProfit;
                    exitReason = 'TIMEOUT';
                }
            }
            ...
            if (exitReason) {
                ...
                trades.push({
                    ...
                    exit: exitReason === 'TIMEOUT' ? k.close : (exitReason === 'WIN' ? pos.tp : pos.sl),
                    result: exitReason,
                    profit: p,
                    balance: balance
                });
    ...
    console.log(`[${timeoutHours}h Timeout] Balance: $${balance.toFixed(2)} | Trades: ${trades.length} | Win: ${trades.filter(t=>t.result==='WIN').length} | Loss: ${trades.filter(t=>t.result==='LOSS').length} | Timeout: ${trades.filter(t=>t.result==='TIMEOUT').length}`);
    return { hours: timeoutHours, balance, trades: trades.length, wins: trades.filter(t=>t.result==='WIN').length, losses: trades.filter(t=>t.result==='LOSS').length, timeouts: trades.filter(t=>t.result==='TIMEOUT').length };
}

async function run() {
    const results = [];
    for (let h = 1; h <= 6; h++) {
        const res = await runBacktest(h);
        results.push(res);
    }
    
    console.log("\n=== FINAL TIMEOUT COMPARISON SUMMARY ===");
