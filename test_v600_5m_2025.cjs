...
async function runV600_5m_2025() {
  console.log(`\n--- [v6.0.0] 5분봉 백테스트 (2025-01-01 ~ 현재) ---`);

  const klines1m_all = await fetchKlines(SYMBOL, '1m', FETCH_START_TIME, ACTUAL_END_TIME);
  const klines5m_all = await fetchKlines(SYMBOL, '5m', FETCH_START_TIME, ACTUAL_END_TIME);
  ...
  const output = ["Time_KST,SignalType,Entry_Price,Entry_Type,Exit_Time,Exit_Price,Result,ROI,Balance,5m_K,5m_D,1h_MACD,1h_Sig,1h_K,1h_D,1d_MACD,1d_Sig"];
  let m5Idx = 0;
    ...
    if (sig !== 'hold' && sig !== currentPosSignal) {
      ...
      if (executed) {
        ...
        for (let j = entryTimeIdx; j < klines1m.length; j++) {
          ...
          if (sig === 'long') {
            if (ex.high >= tpPrice) { balance *= (1 + TARGET_NET_ROI); wins++; output.push(`${entryTimeKST},LONG,${finalEntryPrice},${entryType},${toKSTString(ex.time)},${tpPrice.toFixed(2)},WIN,3.00%,${balance.toFixed(2)},${k5},${d5},${m1h},${s1h},${kh},${dh},${m1d},${s1d}`); i = j; break; }
            if (ex.low <= slPrice) { balance *= (1 - (SL_ROI + (MAKER_FEE_RATE + TAKER_FEE_RATE) * LEVERAGE)); losses++; output.push(`${entryTimeKST},LONG,${finalEntryPrice},${entryType},${toKSTString(ex.time)},${slPrice.toFixed(2)},LOSS,-15.00%,${balance.toFixed(2)},${k5},${d5},${m1h},${s1h},${kh},${dh},${m1d},${s1d}`); i = j; break; }
          } else {
            if (ex.low <= tpPrice) { balance *= (1 + TARGET_NET_ROI); wins++; output.push(`${entryTimeKST},SHORT,${finalEntryPrice},${entryType},${toKSTString(ex.time)},${tpPrice.toFixed(2)},WIN,3.00%,${balance.toFixed(2)},${k5},${d5},${m1h},${s1h},${kh},${dh},${m1d},${s1d}`); i = j; break; }
            if (ex.high >= slPrice) { balance *= (1 - (SL_ROI + (MAKER_FEE_RATE + TAKER_FEE_RATE) * LEVERAGE)); losses++; output.push(`${entryTimeKST},SHORT,${finalEntryPrice},${entryType},${toKSTString(ex.time)},${slPrice.toFixed(2)},LOSS,-15.00%,${balance.toFixed(2)},${k5},${d5},${m1h},${s1h},${kh},${dh},${m1d},${s1d}`); i = j; break; }
          }
        }
  ...
  fs.writeFileSync('C:/dev/2026_candy/backtest_v600_5m_2025_results.csv', output.join('\n'));
  console.log(`\n--- 2025 Results (v6.0.0 - 5m Interval) ---`);
