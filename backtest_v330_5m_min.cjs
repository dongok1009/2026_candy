...
async function runV330_5m_LimitEntry() {
  console.log(`--- [v3.3.0-5m] 백테스트: 지정가 진입 로직 ---`);
  console.log(`조건: 신호 발생 시 직전 5분봉의 Low(Long) / High(Short)로 지정가 진입 시도`);
  ...
  for (let i = 1; i < klines1m.length; i++) {
    ...
    // Get LAST CLOSED 5m candle (ended at or before 'time')
    while (m5Idx < klines5m_all.length && klines5m_all[m5Idx].time + 300000 <= time) m5Idx++;
    const idx5m = m5Idx - 1;

    // Get LAST CLOSED 1h/1d candles
    const r1h = klines1h_all.findIndex(k => k.time + 3600000 > time) - 1;
    const r1d = klines1d_all.findIndex(k => k.time + 86400000 > time) - 1;
    ...
    if (sig !== 'hold' && sig !== currentPosSignal) {
      // LIMIT ENTRY START - Previous 5m Low/High
      const prev5m = klines5m_all[idx5m];
      const limitEntryPrice = sig === 'long' ? prev5m.low : prev5m.high;
      let executed = false;
      let exitIdx = i;
      ...
      if (executed) {
        ...
        for (let j = exitIdx; j < klines1m.length; j++) {
          ...
          if (sig === 'long') {
            if (ex.high >= tpPrice) { balance *= (1 + TARGET_NET_ROI); wins++; output.push(`${entryTimeKST},LONG,${limitEntryPrice},${toKSTString(ex.time)},WIN,3.00%,${balance.toFixed(2)},Executed`); i = j; break; }
            if (ex.low <= slPrice) { balance *= (1 - (SL_ROI + (MAKER_FEE_RATE + TAKER_FEE_RATE) * LEVERAGE)); losses++; output.push(`${entryTimeKST},LONG,${limitEntryPrice},${toKSTString(ex.time)},LOSS,-15.00%,${balance.toFixed(2)},Executed`); i = j; break; }
          } else {
            if (ex.low <= tpPrice) { balance *= (1 + TARGET_NET_ROI); wins++; output.push(`${entryTimeKST},SHORT,${limitEntryPrice},${toKSTString(ex.time)},WIN,3.00%,${balance.toFixed(2)},Executed`); i = j; break; }
            if (ex.high >= slPrice) { balance *= (1 - (SL_ROI + (MAKER_FEE_RATE + TAKER_FEE_RATE) * LEVERAGE)); losses++; output.push(`${entryTimeKST},SHORT,${limitEntryPrice},${toKSTString(ex.time)},LOSS,-15.00%,${balance.toFixed(2)},Executed`); i = j; break; }
          }
        }
  ...
  console.log(`\n--- v3.3.0-5m 결과 (지정가 5% 진입 - 5m Low/High) ---`);
  ...
  fs.writeFileSync('C:/dev/2026_candy/backtest_v330_5m_results.csv', output.join('\n'));
}

runV330_5m_LimitEntry();
