...
const { writeFileAtomicSync } = require('./utils.cjs');
...
function buildRulesFromEnv() {
    ...
    try {
        ...
        const buildIntervalRules = (side, timeframe) => {
            ...
            const useStochExtremeBypassVal = getEnvValue(side, timeframe, 'USE_STOCH_EXTREME_BYPASS');
            const useMaSlopeVal = getEnvValue(side, timeframe, 'USE_MA_SLOPE');
            const useMaRocVal = getEnvValue(side, timeframe, 'USE_MA_ROC');
            const maSlopePeriodVal = getEnvValue(side, timeframe, 'MA_SLOPE_PERIOD');
            const maRocPeriodVal = getEnvValue(side, timeframe, 'MA_ROC_PERIOD');
            const useMaSizeFilterVal = getEnvValue(side, timeframe, 'USE_MA_SIZE_FILTER', 'USE_H1_MA_SIZE_FILTER');
            const useRciCrossVal = getEnvValue(side, timeframe, 'USE_RCI_CROSS');
            const useTrixCrossVal = getEnvValue(side, timeframe, 'USE_TRIX_CROSS');
            const trixSignalPeriodVal = getEnvValue(side, timeframe, 'TRIX_SIGNAL_PERIOD');
            const rciLongPeriodVal = getEnvValue(side, timeframe, 'RCI_LONG_PERIOD');
            ...
            return {
                ...
                stochExtremeBypassEnabled: toBool(useStochExtremeBypassVal),
                ...
                useMaSlope: toBool(useMaSlopeVal),
                useMaRoc: toBool(useMaRocVal),
                maSlopePeriod: toNum(maSlopePeriodVal, 20),
                maRocPeriod: toNum(maRocPeriodVal, 20),
                useMaSizeFilter: toBool(useMaSizeFilterVal),
                useRciCross: toBool(useRciCrossVal),
                useTrixCross: toBool(useTrixCrossVal),
                trixSignalPeriod: toNum(trixSignalPeriodVal, 9),
                rciLongPeriod: toNum(rciLongPeriodVal, 26)
            };
        };
        ...
        return {
            ...
            "global": {
                ...
                "switchingEnabled": env.SWITCHING_ENABLED !== undefined ? toBool(env.SWITCHING_ENABLED) : undefined,
                "maSlopeAlign5mEnabled": env.MA_SLOPE_ALIGN_5M_ENABLED !== undefined ? toBool(env.MA_SLOPE_ALIGN_5M_ENABLED) : (env.USE_MA_SLOPE_ALIGN_5M !== undefined ? toBool(env.USE_MA_SLOPE_ALIGN_5M) : false),
                "maSlopeAlign1hEnabled": env.MA_SLOPE_ALIGN_1H_ENABLED !== undefined ? toBool(env.MA_SLOPE_ALIGN_1H_ENABLED) : (env.USE_MA_SLOPE_ALIGN_1H !== undefined ? toBool(env.USE_MA_SLOPE_ALIGN_1H) : false),
                "maSlopeAlignPeriod5m": env.MA_SLOPE_ALIGN_PERIOD_5M !== undefined ? toNum(env.MA_SLOPE_ALIGN_PERIOD_5M, 20) : (env.MA_SLOPE_ALIGN_PERIOD !== undefined ? toNum(env.MA_SLOPE_ALIGN_PERIOD, 20) : 20),
                "maSlopeAlignPeriod5m": env.MA_SLOPE_ALIGN_PERIOD_5M !== undefined ? toNum(env.MA_SLOPE_ALIGN_PERIOD_5M, 20) : (env.MA_SLOPE_ALIGN_PERIOD !== undefined ? toNum(env.MA_SLOPE_ALIGN_PERIOD, 20) : 20),
                "maSlopeAlignPeriod1h": env.MA_SLOPE_ALIGN_PERIOD_1H !== undefined ? toNum(env.MA_SLOPE_ALIGN_PERIOD_1H, 20) : (env.MA_SLOPE_ALIGN_PERIOD !== undefined ? toNum(env.MA_SLOPE_ALIGN_PERIOD, 20) : 20),
                "whatIfFilters": (() => {
                    if (!env.WHAT_IF_FILTERS) return [];
                    try {
                        return JSON.parse(env.WHAT_IF_FILTERS);
                    } catch (e) {
                        console.error("❌ [RulesHelper] Failed to parse WHAT_IF_FILTERS:", e.message);
                        return [];
                    }
                })()
            }
...
function ensureEnvTemplate() {
    ...
    try {
        ...
        if (envContent.includes('5M_USE_ADX') || envContent.includes('5M_LONG_USE_ADX')) {
            return; // 이미 템플릿 존재함
        }
...
function syncRulesToEnv(rules) {
    ...
    try {
        ...
        if (rules.global) {
            ...
            if (g.switchingEnabled !== undefined) updates['SWITCHING_ENABLED'] = g.switchingEnabled;
            if (g.maSlopeAlign5mEnabled !== undefined) updates['MA_SLOPE_ALIGN_5M_ENABLED'] = g.maSlopeAlign5mEnabled;
            if (g.maSlopeAlign1hEnabled !== undefined) updates['MA_SLOPE_ALIGN_1H_ENABLED'] = g.maSlopeAlign1hEnabled;
            if (g.maSlopeAlignPeriod5m !== undefined) updates['MA_SLOPE_ALIGN_PERIOD_5M'] = g.maSlopeAlignPeriod5m;
            if (g.maSlopeAlignPeriod1h !== undefined) updates['MA_SLOPE_ALIGN_PERIOD_1H'] = g.maSlopeAlignPeriod1h;
            if (g.whatIfFilters !== undefined) {
                updates['WHAT_IF_FILTERS'] = JSON.stringify(g.whatIfFilters);
            }
        }
        ...
        for (const tf of timeframes) {
            ...
            const legacyKeys = [
                ...
                `USE_STOCH_EXTREME_BYPASS`,
                `USE_RSI`, `RSI_LOW`, `RSI_HIGH`,
                `USE_MACD_VALUE`, `USE_MACD_VAL`, `MACD_VALUE_THRESHOLD`, `MACD_VAL`,
                `USE_MA_SLOPE`, `MA_SLOPE_PERIOD`, `USE_MA_ROC`, `MA_ROC_PERIOD`, `USE_MA_SIZE_FILTER`, `USE_H1_MA_SIZE_FILTER`,
                `USE_RCI_CROSS`, `USE_TRIX_CROSS`, `TRIX_SIGNAL_PERIOD`, `RCI_LONG_PERIOD`
            ];
        ...
        for (const side of sides) {
            for (const tf of timeframes) {
                ...
                const extremeBypass = r.useStochExtremeBypass !== undefined ? r.useStochExtremeBypass : r.stochExtremeBypassEnabled;
                if (extremeBypass !== undefined) updates[`${prefix}_USE_STOCH_EXTREME_BYPASS`] = extremeBypass;
                ...
                if (r.useMaSlope !== undefined) updates[`${prefix}_USE_MA_SLOPE`] = r.useMaSlope;
                if (r.maSlopePeriod !== undefined) updates[`${prefix}_MA_SLOPE_PERIOD`] = r.maSlopePeriod;
                if (r.useMaRoc !== undefined) updates[`${prefix}_USE_MA_ROC`] = r.useMaRoc;
                if (r.maRocPeriod !== undefined) updates[`${prefix}_MA_ROC_PERIOD`] = r.maRocPeriod;
                if (r.useMaSizeFilter !== undefined) updates[`${prefix}_USE_MA_SIZE_FILTER`] = r.useMaSizeFilter;

                if (r.useRciCross !== undefined) updates[`${prefix}_USE_RCI_CROSS`] = r.useRciCross;
                if (r.useTrixCross !== undefined) updates[`${prefix}_USE_TRIX_CROSS`] = r.useTrixCross;
                if (r.trixSignalPeriod !== undefined) updates[`${prefix}_TRIX_SIGNAL_PERIOD`] = r.trixSignalPeriod;
                if (r.rciLongPeriod !== undefined) updates[`${prefix}_RCI_LONG_PERIOD`] = r.rciLongPeriod;
            }
        }
        ...
        // 원자적 저장 — 쓰기가 실패해도 기존 .env가 잘린 채 남지 않는다.
        writeFileAtomicSync(ENV_PATH, envContent);
        return true;
    } catch (err) {
...
/**
 * 진입 시그널 발생 시 WHAT-IF 조건을 평가하여 조정 배율 및 차단 여부를 산출합니다.
 */
function evaluateWhatIfFilters(side, refIndices, indicators, overrideRules) {
    const result = {
        isBlocked: false,
        sizeMultiplier: 1.0,
        tpMultiplier: 1.0,
    ...
    if (!overrideRules || !overrideRules.global || !overrideRules.global.whatIfFilters) {
        return result;
    }

    const filters = overrideRules.global.whatIfFilters;
    const currentSide = side.toLowerCase(); // 'long' or 'short'

    for (const filter of filters) {
        // 1. 방향 필터링
        const filterSide = filter.side ? filter.side.toLowerCase() : 'both';
        if (filterSide !== 'both' && filterSide !== currentSide) {
            continue;
        }

        // 2. 타임프레임 및 인덱스 맵핑
        const tf = filter.timeframe; // '5m' or '1h' or '1d'
        const tfKey = tf === '5m' ? 'm5' : (tf === '1h' ? 'h1' : (tf === '1d' ? 'd1' : tf));
        
        let idx = 0;
        if (tfKey === 'm5') idx = refIndices.idx5m;
        else if (tfKey === 'h1') idx = refIndices.r1h;
        else if (tfKey === 'd1') idx = refIndices.r1d;
        else continue;

        const tfData = indicators[tfKey];
        if (!tfData) continue;

        // 3. 지표값 추출
        const indName = filter.indicator ? filter.indicator.toLowerCase().replace(/\s+/g, '_') : '';
        let indicatorVal = null;

        // 지표 이름 매핑 유연화 (5ma_adx ➔ adx, 5ma_slope ➔ ma_slope_5 등)
        if ((indName === 'adx' || indName === '5ma_adx') && tfData.adx) {
            indicatorVal = tfData.adx[idx];
        } else if (indName === 'rsi' && tfData.rsi) {
            indicatorVal = tfData.rsi[idx];
        } else if (indName === 'macd' && tfData.macd) {
            indicatorVal = tfData.macd.m[idx];
        } else if (indName === 'macd_sig' && tfData.macd) {
            indicatorVal = tfData.macd.s[idx];
        } else if ((indName === 'stochk' || indName === 'stochrsi_k') && tfData.stoch) {
            indicatorVal = tfData.stoch.k[idx];
        } else if ((indName === 'stochd' || indName === 'stochrsi_d') && tfData.stoch) {
            indicatorVal = tfData.stoch.d[idx];
        } else if (indName === 'bbw' && tfData.bbw) {
            indicatorVal = tfData.bbw[idx];
        } else if (indName === 'bbwp' && tfData.bbwp) {
            indicatorVal = tfData.bbwp[idx];
        } else if ((indName === 'ma_slope_3' || indName === '3ma_slope') && tfData.ma_slope_3) {
            indicatorVal = tfData.ma_slope_3[idx];
        } else if ((indName === 'ma_slope_5' || indName === '5ma_slope') && tfData.ma_slope_5) {
            indicatorVal = tfData.ma_slope_5[idx];
        } else if (indName === 'ma_slope_10' && tfData.ma_slope_10) {
            indicatorVal = tfData.ma_slope_10[idx];
        } else if (indName === 'ma_slope_20' && tfData.ma_slope_20) {
            indicatorVal = tfData.ma_slope_20[idx];
        } else if (indName === 'ma_roc' && tfData.ma_roc) {
            indicatorVal = tfData.ma_roc[idx];
        } else {
            // 지표 객체에서 직접 조회 시도 (예: ma_slope_5, adx 등)
            if (tfData[indName]) {
                indicatorVal = tfData[indName][idx];
            } else if (tfData[filter.indicator]) {
                indicatorVal = tfData[filter.indicator][idx];
            }
        }

        if (indicatorVal === null || indicatorVal === undefined) {
            continue;
        }
        ...
        let isMatched = false;
        if (op === '>') {
            if (indicatorVal > threshold) isMatched = true;
        } else if (op === '<') {
            if (indicatorVal < threshold) isMatched = true;
        } else if (op === '=') {
            if (Math.abs(indicatorVal - threshold) < 1e-6) isMatched = true;
        }

        // 5. 조건 매칭 시 액션 실행
        if (isMatched) {
            const action = filter.action ? filter.action.toLowerCase() : '';
            console.log(`[WHAT-IF MATCHED] Side:${currentSide}, Filter:${tf} ${indName} ${op} ${threshold} (CurrentVal:${indicatorVal.toFixed(4)}) ➔ Action:${action}`);
            if (action === 'block' || action === '차단') {
                result.isBlocked = true;
            } else if (action === 'size_50' || action === '50%진입' || action === '비중조정') {
                result.sizeMultiplier *= 0.5;
            } else if (action === 'tp_50' || action === 'tp 50%축소' || action === '익절조정') {
                result.tpMultiplier *= 0.5;
            } else if (action === 'sl_50' || action === 'sl 50%축소' || action === '손절조정') {
                result.slMultiplier *= 0.5;
            }
        }
    }
