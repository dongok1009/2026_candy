  ...
  for (let y = startYear; y <= endYear; y++) {
      ...
      if (!cacheLoaded || yearData.length === 0) {
          ...
          if (updated) {
              ...
              try {
                  const retried = await writeFileAtomicWithRetry(cacheFile, JSON.stringify(yearData, null, 2));
                  const suffix = retried > 0 ? ` (재시도 ${retried}회)` : '';
                  console.log(`💾 [CACHE UPDATE] Saved merged cache for ${y} containing ${yearData.length} klines.${suffix}`);
              } catch (err) {
...
// 파일을 원자적으로 저장한다. 임시 파일에 쓴 뒤 rename으로 교체하므로
// 쓰기가 중간에 실패해도 기존 파일이 손상되지 않는다. 실패 시 예외를 던진다.
function writeFileAtomicSync(filePath, contents) {
  ...
  try {
    ...
    try { if (fs.existsSync(tmpPath)) fs.unlinkSync(tmpPath); } catch { /* 정리 실패는 무시 */ }
    throw err;
  }
}

// 위와 같되 실패 시 잠시 기다렸다 재시도한다. 이 환경에서 쓰기가 간헐적으로
// UNKNOWN 오류를 내고 잠시 후 스스로 복구되기 때문이다.
// 대기를 동기(busy wait)로 하면 서버 이벤트 루프가 통째로 멈추므로 반드시 비동기다.
// 성공한 시도 번호를 반환한다(0이면 첫 시도에 성공).
async function writeFileAtomicWithRetry(filePath, contents, retries = 3, delayMs = 200) {
  let lastErr;
