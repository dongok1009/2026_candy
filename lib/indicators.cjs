...
const calculateSlope = (data) => {
  const slope = new Array(data.length).fill(null);
...
// null이 있는 원본 배열(src)의 non-null 위치에 압축값(vals)을 순서대로 되돌려 정렬 (중첩 EMA용, MACD 시그널 정렬과 동일 방식)
...
const calculateRCI = (closes, p = 9) => {
  const out = new Array(closes.length).fill(null);
  if (p < 2) return out;
  const denom = p * (p * p - 1); // p^3 - p
  for (let i = p - 1; i < closes.length; i++) {
    // 최근봉(j=0) timeRank=1 ... 오래된봉 timeRank=p
    const win = [];
    for (let j = 0; j < p; j++) win.push({ price: closes[i - j], timeRank: j + 1, priceRank: 0 });
    // 가격 높은 순으로 priceRank 1..p (동점은 평균 순위)
...
const calculateTRIX = (closes, p = 14, signalPeriod = 9) => {
  const e1 = calculateEMA(closes, p);
  const e2 = realignTo(e1, calculateEMA(e1.filter(v => v !== null), p));
  const e3 = realignTo(e2, calculateEMA(e2.filter(v => v !== null), p));
