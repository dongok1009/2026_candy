        ...
        for (let j = entryTimeIdx + 1; j < klines1m.length; j++) {
            ...
            if (exitReason) {
                ...
                const tradeData = {
                    ...
                    m5_ma_slope: indicators.m5?.ma_slope?.[finalIdx5m]?.toFixed(4) || '-',
                    h1_ma_slope: indicators.h1?.ma_slope?.[finalR1h]?.toFixed(4) || '-',
                    d1_ma_slope: indicators.d1?.ma_slope?.[finalR1d]?.toFixed(4) || '-',
                    m5_ma_roc: indicators.m5?.ma_roc?.[finalIdx5m]?.toFixed(1) || '-',
                    h1_ma_roc: indicators.h1?.ma_roc?.[finalR1h]?.toFixed(1) || '-',
                    d1_ma_roc: indicators.d1?.ma_roc?.[finalR1d]?.toFixed(1) || '-',
                ...
                const sm5maslope = indicators.m5?.ma_slope?.[sigIdx5m]?.toFixed(4) || '-';
                const sh1maslope = indicators.h1?.ma_slope?.[sigR1h]?.toFixed(4) || '-';
                const sd1maslope = indicators.d1?.ma_slope?.[sigR1d]?.toFixed(4) || '-';
                const sm5maroc = indicators.m5?.ma_roc?.[sigIdx5m]?.toFixed(1) || '-';
                const sh1maroc = indicators.h1?.ma_roc?.[sigR1h]?.toFixed(1) || '-';
                const sd1maroc = indicators.d1?.ma_roc?.[sigR1d]?.toFixed(1) || '-';
