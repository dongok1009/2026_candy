    ...
    for (let i = 1; i < klines1m.length; i++) {
        ...
        for (let j = entryTimeIdx + 1; j < klines1m.length; j++) {
            ...
            if (exitReason) {
                ...
                const isTakerExit = !(exitReason === 'TP' || exitReason === 'TIMEOUT');
                const feeRate = isTakerExit ? (config.MAKER_FEE_RATE + config.TAKER_FEE_RATE) : (config.MAKER_FEE_RATE + config.EXIT_MAKER_FEE_RATE);
                // [SLIPPAGE] 시장가(taker) 청산은 트리거가보다 불리하게 체결됨. 손실 방향 슬리피지를 순ROI에서 차감(수수료와 동일 환산). TP/TIMEOUT(지정가/maker)은 제외.
                const exitSlippage = (isTakerExit && config.EXIT_SLIPPAGE_RATE !== undefined) ? parseFloat(config.EXIT_SLIPPAGE_RATE) : 0;
                ...
                if (exitReason === 'TP') {
                    ...
                    finalNetRoi = -((config.SL_ROI * whatIf.slMultiplier) + ((feeRate + exitSlippage) * config.LEVERAGE));
                } else { // TIMEOUT / SWITCHING / TRAIL (시장가 차익 계산)
                    const priceDiff = side === 'LONG' ? (exitPrice / entryPrice - 1) : (entryPrice / exitPrice - 1);
                    finalNetRoi = (priceDiff * config.LEVERAGE) - ((feeRate + exitSlippage) * config.LEVERAGE);
                }
