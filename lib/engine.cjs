...
const { calculateEMA, calculateSMA, calculateRSI, calculateMACD, calculateStochRSI, calculateADX, calculateBBW, calculateBBWP, calculateROC, calculateSlope } = require('./indicators.cjs');
...
async function runBacktest(strategy) {
    ...
    const m1Periods = new Set([20]); // 기본값 20은 상시 보장
    ...
    const fullValidationRows = [
        ...metadataLines,
        "Trade#,Side,Status,Time,Open,High,Low,Close,EntryBasis,EntryPrice,TargetTP,TargetSL,M5_StochK,M5_StochD,M5_ADX,M5_RSI,M5_MACD,M5_MACDSig,M5_BBW,M5_BBWP,M5_BBW_ROC,M5_MA_Slope,M5_MA_ROC,Conf_H1_MACD,Conf_H1_Sig,Conf_H1_StochK,Conf_H1_StochD,Conf_H1_RSI,H1_MACD,H1_MACDSig,H1_StochK,H1_StochD,H1_ADX,H1_RSI,H1_BBW,H1_BBWP,H1_BBW_ROC,H1_MA_Slope,H1_MA_ROC,D1_MACD,D1_MACDSig,D1_StochK,D1_StochD,D1_ADX,D1_RSI,D1_BBW,D1_BBWP,D1_BBW_ROC,D1_MA_Slope,D1_MA_ROC"
    ];
    ...
    for (let i = 1; i < klines1m.length; i++) {
        ...
        for (let j = entryTimeIdx + 1; j < klines1m.length; j++) {
            ...
            if (exitReason) {
                ...
                } else { // TIMEOUT / SWITCHING (시장가 차익 계산)
                ...
                const fundingFee = (balance * config.LEVERAGE * config.FUNDING_FEE_RATE * sizeMultiplier); // 샘플 펀딩피
                ...
                const getPeriod = (tf, type, sideStr) => {
                    ...
                    if (type === 'slope') {
                        return r && r.maSlopePeriod !== undefined ? parseInt(r.maSlopePeriod) : 20;
                    } else {
                        return r && r.maRocPeriod !== undefined ? parseInt(r.maRocPeriod) : 20;
                    }
                };
                ...
                const h1_ma_slope_val = getM1Indicator('ma_slope', h1SlopeP, entryTimeIdx);
                const h1_ma_roc_val = getM1Indicator('ma_roc', h1RocP, entryTimeIdx);
                const d1_ma_slope_val = getM1Indicator('ma_slope', d1SlopeP, entryTimeIdx);
                const d1_ma_roc_val = getM1Indicator('ma_roc', d1RocP, entryTimeIdx);
                ...
                const tradeData = {
                    ...
                    m5_ma_slope: m5_ma_slope_val !== null ? m5_ma_slope_val.toFixed(4) : '-',
                    h1_ma_slope: h1_ma_slope_val !== null ? h1_ma_slope_val.toFixed(4) : '-',
                    d1_ma_slope: d1_ma_slope_val !== null ? d1_ma_slope_val.toFixed(4) : '-',
                ...
                const sm5maslope_val = getM1Indicator('ma_slope', m5SlopeP, i);
                const sh1maslope_val = getM1Indicator('ma_slope', h1SlopeP, i);
                const sd1maslope_val = getM1Indicator('ma_slope', d1SlopeP, i);
                const sm5maroc_val = getM1Indicator('ma_roc', m5RocP, i);
                const sh1maroc_val = getM1Indicator('ma_roc', h1RocP, i);
                const sd1maroc_val = getM1Indicator('ma_roc', d1RocP, i);

                const sm5maslope = sm5maslope_val !== null ? sm5maslope_val.toFixed(4) : '-';
                const sh1maslope = sh1maslope_val !== null ? sh1maslope_val.toFixed(4) : '-';
                const sd1maslope = sd1maslope_val !== null ? sd1maslope_val.toFixed(4) : '-';
                ...
                signalRows.push([
                    ...
                    sm3k, sm3d, sm3adx,
                    // m5 지표
                    sm5k, sm5d, sm5adx, sm5rsi, sm5m, sm5s, sm5bbw, sm5bbwp, sm5bbwroc, sm5maslope, sm5maroc,
                    // h1 지표
                    sh1m, sh1s, sh1k, sh1d, sh1adx, sh1rsi, sh1bbw, sh1bbwp, sh1bbwroc, sh1maslope, sh1maroc,
                    // h12 지표
                    sh12m, sh12s, sh12k, sh12d, sh12adx,
                    // d1 지표
                    sd1m, sd1s, sd1k, sd1d, sd1adx, sd1rsi, sd1bbw, sd1bbwp, sd1bbwroc, sd1maslope, sd1maroc
                ].join(','));
                ...
                try {
                    ...
                    for (let vIdx = valStartTimeIdx; vIdx <= valEndTimeIdx; vIdx++) {
                        ...
                        fullValidationRows.push([
                            ...
                            indicators.m5?.ma_slope?.[vIdx5m]?.toFixed(4) || '-',
                            ...
                            indicators.h1?.ma_slope?.[vR1h]?.toFixed(4) || '-',
                            ...
                            indicators.d1?.ma_slope?.[vR1d]?.toFixed(4) || indicators.d1?.ma_slope?.slice(0, vR1d).reverse().find(v => v !== null)?.toFixed(4) || '-',
                ...
                output.push([
                    ...
                    tradeData.m5_ma_slope, tradeData.m5_ma_roc,
                    // h1 지표
                    h1m, h1s, h1k, h1d, h1adx, h1rsi,
                    tradeData.h1_bbw, tradeData.h1_bbwp, tradeData.h1_bbw_roc,
                    tradeData.h1_ma_slope, tradeData.h1_ma_roc,
                    // h12 지표
                    ...
                    tradeData.d1_ma_slope, tradeData.d1_ma_roc
                ].join(','));
