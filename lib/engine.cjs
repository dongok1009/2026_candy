...
async function runBacktest(strategy) {
    ...
    const klines12h_all = await fetchKlines(config.SYMBOL, '12h', config.FETCH_START_TIME, config.END_TIME);
    ...
    const output = [
        "entryTime,exitTime,balance,cumRoi,side,entryBasis,entryPrice,exitPrice,netProfit,roe,quantity,fee,fundingFee,m5_stochK,m5_stochD,m5_adx,h1_macd,h1_macdSig,h1_stochK,h1_stochD,h1_adx,h12_macd,h12_macdSig,h12_stochK,h12_stochD,h12_adx,d1_macd,d1_macdSig,d1_stochK,d1_stochD,d1_adx"
    ];
    ...
    for (let i = 1; i < klines1m.length; i++) {
        ...
        if (idx5m < 0 || r1h < 0 || (r1d < 0 && r12h < 0)) continue;

        // [핵심] 대시보드 설정을 overrideRules로 전달하여 동적 신호 판단
        const signal = signal_logic(indicators, { idx5m, r1h, r12h, r1d, idx1m: i - 1 }, config.overrideRules);
        ...
        for (let j = entryTimeIdx + 1; j < klines1m.length; j++) {
            ...
            if (exitReason) {
                ...
                // CSV 출력 컬럼 순서 연동
                output.push(`${entryTimeKST},${exitTimeKST},${balance.toFixed(2)},${cumRoi}%,${side},${tradeData.entryBasis},${entryPrice},${exitPrice},${prof.toFixed(2)},${tradeData.roe},${qty.toFixed(4)},${totalFee.toFixed(2)},${fundingFee.toFixed(2)},${m5k},${m5d},${m5adx},${h1m},${h1s},${h1k},${h1d},${h1adx},${h12m},${h12s},${h12k},${h12d},${h12adx},${tradeData.d1_macd},${tradeData.d1_macdSig},${tradeData.d1_stochK},${tradeData.d1_stochD},${tradeData.d1_adx}`);
