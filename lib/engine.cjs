...
async function runBacktest(strategy) {
    ...
    const output = [
        "entryTime,exitTime,balance,cumRoi,side,entryBasis,entryPrice,exitPrice,netProfit,roe,quantity,fee,fundingFee,m3_stochK,m3_stochD,m3_adx,m5_stochK,m5_stochD,m5_adx,m5_rsi,h1_macd,h1_macdSig,h1_stochK,h1_stochD,h1_adx,h1_rsi,h12_macd,h12_macdSig,h12_stochK,h12_stochD,h12_adx,d1_macd,d1_macdSig,d1_stochK,d1_stochD,d1_adx,d1_rsi"
    ];

    // [New] 통합 검증 리포트용 헤더 (방향, 진입가 포함)
    const fullValidationRows = [
        "Trade#,Side,Status,Time,Open,High,Low,Close,EntryBasis,EntryPrice,TargetTP,TargetSL,M5_StochK,M5_StochD,M5_ADX,M5_RSI,M5_MACD,M5_MACDSig,Conf_H1_MACD,Conf_H1_Sig,Conf_H1_StochK,Conf_H1_StochD,Conf_H1_RSI,H1_MACD,H1_MACDSig,H1_StochK,H1_StochD,H1_ADX,H1_RSI,D1_MACD,D1_MACDSig,D1_StochK,D1_StochD,D1_ADX,D1_RSI"
    ];
        ...
        for (let j = entryTimeIdx + 1; j < klines1m.length; j++) {
            ...
            if (exitReason) {
                ...
                const tradeData = {
                    ...
                    m5_stochK: m5k, m5_stochD: m5d, m5_adx: m5adx, m5_rsi: m5rsi,
                ...
                // CSV 출력 컬럼 순서 연동
                output.push(`${entryTimeKST},${exitTimeKST},${balance.toFixed(2)},${cumRoi}%,${side},${tradeData.entryBasis},${entryPrice},${exitPrice},${prof.toFixed(2)},${tradeData.roe},${qty.toFixed(4)},${totalFee.toFixed(2)},${fundingFee.toFixed(2)},${m3k},${m3d},${m3adx},${m5k},${m5d},${m5adx},${m5rsi},${h1m},${h1s},${h1k},${h1d},${h1adx},${h1rsi},${h12m},${h12s},${h12k},${h12d},${h12adx},${tradeData.d1_macd},${tradeData.d1_macdSig},${tradeData.d1_stochK},${tradeData.d1_stochD},${tradeData.d1_adx},${tradeData.d1_rsi}`);
