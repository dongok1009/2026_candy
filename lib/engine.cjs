...
async function runBacktest(strategy) {
    ...
    const output = [
        "entryTime,exitTime,balance,cumRoi,side,entryBasis,entryPrice,exitPrice,netProfit,roe,quantity,fee,fundingFee,m5_stochK,m5_stochD,m5_adx,h1_macd,h1_macdSig,h1_stochK,h1_stochD,h1_adx,d1_macd,d1_macdSig,d1_adx"
    ];
    ...
    for (let i = 1; i < klines1m.length; i++) {
        ...
        const k5_confirmed = klines5m_all[idx5m - 1]; // 현재 시점에 완전히 종료된 이전 5분봉
        ...
        for (let j = entryTimeIdx + 1; j < klines1m.length; j++) {
            ...
            if (exitReason) {
                ...
                const finalR1d = klines1d_all.findIndex(k => k.time > klines1m[entryTimeIdx].time - 86400000) - 1;
                ...
                const tradeData = {
                    ...
                    exitPrice, 
                    exitReason,
                    entryBasis: entryResult.entryBasis,
                ...
                try {
                    ...
                    for (let vIdx = valStartTimeIdx; vIdx <= valEndTimeIdx; vIdx++) {
                        ...
                        const vTime = vk.time;
                        const vIdx5m = klines5m_all.findIndex(k => k.time >= vTime) - 1;
                        const vR1h = klines1h_all.findIndex(k => k.time > vTime - 3600000) - 1;
                        const vR1d = klines1d_all.findIndex(k => k.time > vTime - 86400000) - 1;
                        ...
                        fullValidationRows.push([
                            ...
                            indicators.d1?.macd?.m?.[vR1d]?.toFixed(2) || '-',
                            indicators.d1?.macd?.s?.[vR1d]?.toFixed(2) || '-',
                            indicators.d1?.stoch?.k?.[vR1d]?.toFixed(1) || '-',
                            indicators.d1?.stoch?.d?.[vR1d]?.toFixed(1) || '-',
                            indicators.d1?.adx?.[vR1d]?.toFixed(1) || '-'
                        ].join(','));
                    }
                ...
                // CSV 출력 컬럼 순서 연동: #,진입,청산,잔액,누적ROI,방향,진입근거,진입가,청산가,순수익,ROE,수량,수수료,펀딩피... (필요에 따라 순서 조정)
                output.push(`${entryTimeKST},${exitTimeKST},${balance.toFixed(2)},${cumRoi}%,${side},${tradeData.entryBasis},${entryPrice},${exitPrice},${prof.toFixed(2)},${tradeData.roe},${qty.toFixed(4)},${totalFee.toFixed(2)},${fundingFee.toFixed(2)},${m5k},${m5d},${m5adx},${h1m},${h1s},${h1k},${h1d},${h1adx},${tradeData.d1_macd},${tradeData.d1_macdSig},${tradeData.d1_adx}`);
