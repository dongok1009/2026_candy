...
async function runBacktest(strategy) {
    ...
    console.log("Aggregating timeframes (3m, 5m, 1h, 12h, 1d)...");
    ...
    const output = [
        "entryTime,exitTime,balance,cumRoi,side,entryBasis,entryPrice,exitPrice,netProfit,roe,quantity,fee,fundingFee,m3_stochK,m3_stochD,m3_adx,m5_stochK,m5_stochD,m5_adx,h1_macd,h1_macdSig,h1_stochK,h1_stochD,h1_adx,h12_macd,h12_macdSig,h12_stochK,h12_stochD,h12_adx,d1_macd,d1_macdSig,d1_stochK,d1_stochD,d1_adx"
    ];

    // [New] 통합 검증 리포트용 헤더 (방향, 진입가 포함)
    const fullValidationRows = [
        "Trade#,Side,Status,Time,Open,High,Low,Close,EntryBasis,EntryPrice,TargetTP,TargetSL,M5_StochK,M5_StochD,M5_ADX,M5_MACD,M5_MACDSig,Conf_H1_MACD,Conf_H1_Sig,Conf_H1_StochK,Conf_H1_StochD,H1_MACD,H1_MACDSig,H1_StochK,H1_StochD,H1_ADX,D1_MACD,D1_MACDSig,D1_StochK,D1_StochD,D1_ADX"
    ];
    ...
    for (let i = 1; i < klines1m.length; i++) {
        ...
        // 상위 봉 인덱스 계산
        const idx3m = klines3m_all.findLastIndex(k => k.time + 180000 <= referenceTime + 60000);
        ...
        const r12h = klines12h_all.findLastIndex(k => k.time + (12 * 3600000) <= referenceTime + 60000);
        ...
        for (let j = entryTimeIdx + 1; j < klines1m.length; j++) {
            ...
            if (exitReason) {
                ...
                const finalIdx3m = klines3m_all.findIndex(k => k.time >= klines1m[entryTimeIdx].time) - 1;
                ...
                const finalR12h = klines12h_all.findLastIndex(k => k.time + (12 * 3600000) <= klines1m[entryTimeIdx].time);
                ...
                // CSV 출력 컬럼 순서 연동
                output.push(`${entryTimeKST},${exitTimeKST},${balance.toFixed(2)},${cumRoi}%,${side},${tradeData.entryBasis},${entryPrice},${exitPrice},${prof.toFixed(2)},${tradeData.roe},${qty.toFixed(4)},${totalFee.toFixed(2)},${fundingFee.toFixed(2)},${m3k},${m3d},${m3adx},${m5k},${m5d},${m5adx},${h1m},${h1s},${h1k},${h1d},${h1adx},${h12m},${h12s},${h12k},${h12d},${h12adx},${tradeData.d1_macd},${tradeData.d1_macdSig},${tradeData.d1_stochK},${tradeData.d1_stochD},${tradeData.d1_adx}`);
