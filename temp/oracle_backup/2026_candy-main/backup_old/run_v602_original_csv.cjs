...
const resFile = `backtest_v602_original_2025_ledger.csv`;
...
async function run() {
  ...
  for(let i=0; i<k1m.length; i++){
    ...
    if(pos) {
      let profit = 0;
      if(pos.side==='LONG'){
        if(k.high>=pos.tp){ profit = balance*TP_NET; balance+=profit; trades.push({time:new Date(pos.time).toLocaleString(), type:pos.side, entry:pos.entryPrice, exit:pos.tp, result:'WIN', profit, balance}); pos=null; }
        else if(k.low<=pos.sl){ profit = balance*(-FIXED_SL-0.0035); balance+=profit; trades.push({time:new Date(pos.time).toLocaleString(), type:pos.side, entry:pos.entryPrice, exit:pos.sl, result:'LOSS', profit, balance}); pos=null; }
      } else {
        if(k.low<=pos.tp){ profit = balance*TP_NET; balance+=profit; trades.push({time:new Date(pos.time).toLocaleString(), type:pos.side, entry:pos.entryPrice, exit:pos.tp, result:'WIN', profit, balance}); pos=null; }
        else if(k.high>=pos.sl){ profit = balance*(-FIXED_SL-0.0035); balance+=profit; trades.push({time:new Date(pos.time).toLocaleString(), type:pos.side, entry:pos.entryPrice, exit:pos.sl, result:'LOSS', profit, balance}); pos=null; }
      }
    }
  }

  const header = "Time,Type,EntryPrice,ExitPrice,Result,Profit,Balance\n";
  const rows = trades.map(t => `${t.time},${t.type},${t.entry.toFixed(2)},${t.exit.toFixed(2)},${t.result},${t.profit.toFixed(2)},${t.balance.toFixed(2)}`).join("\n");
  fs.writeFileSync(resFile, header + rows);
  console.log(`\n✅ v6.0.2 Original CSV Complete!`);
