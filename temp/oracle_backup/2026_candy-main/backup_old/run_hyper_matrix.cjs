...
function backtest(k1m, i5m, i1h, i1d, k5m, k1h, k1d, retraceP, waitM) {
...
console.log(`| Retrace | Wait (min) | Final Balance | Win Rate | Trades |`);
console.log(`| :--- | :--- | :--- | :--- | :--- |`);

for(let r of retraces) {
  for(let w of waits) {
    const res = backtest(k1m, i5m, i1h, i1d, k5m, k1h, k1d, r, w);
    const wr = (res.wins/(res.wins+res.losses)*100).toFixed(1);
    console.log(`| ${r}% | ${w}m | $${res.balance.toFixed(2)} | ${wr}% | ${res.wins+res.losses} |`);
  }
}
