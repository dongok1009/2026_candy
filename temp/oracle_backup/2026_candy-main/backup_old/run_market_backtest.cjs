...
const FETCH_START = START_TIME - (86400000 * 180); 
...
async function run() {
  ...
  let balance = 1000, pos = null, m5Idx = 0, currentGSignal = 'HOLD';
  const LEV = 5, TP_NET = 0.03, SL_NET = 0.15;
  const SLIPPAGE = 0.0005; // 0.05% slippage for market entry
  const FEE_MARGIN = (0.0002+0.0002+SLIPPAGE)*LEV; // Including slippage in cost

  const tradeLog = [];
  let wins = 0, losses = 0;

  console.log('Running Market Entry (v6.0.0 Idealistic) simulation...');
  for(let i=0; i<k1m.length; i++){
    ...
    // Market Entry Logic (v6.0.0 Style)
    if(!pos && gSig !== 'HOLD' && gSig !== currentGSignal) {
      // Entry at currently opening 1m candle's Open price (as soon as signal is confirmed)
      const entry = k.open; 
      if(gSig==='LONG'){
    ...
    if(pos) {
      if(pos.side==='LONG'){
        if(k.high>=pos.tp){
          balance*=(1+TP_NET); tradeLog.push({type:'LONG', entry:pos.entryPrice, exit:pos.tp, roi:3.0, time:toKST(pos.entryTime)});
          wins++; pos=null;
        } else if(k.low<=pos.sl){
          balance*=(1-(SL_NET+0.0007*LEV)); tradeLog.push({type:'LONG', entry:pos.entryPrice, exit:pos.sl, roi:-15.0, time:toKST(pos.entryTime)});
          losses++; pos=null;
        }
      } else {
        if(k.low<=pos.tp){
          balance*=(1+TP_NET); tradeLog.push({type:'SHORT', entry:pos.entryPrice, exit:pos.tp, roi:3.0, time:toKST(pos.entryTime)});
          wins++; pos=null;
        } else if(k.high>=pos.sl){
          balance*=(1-(SL_NET+0.0007*LEV)); tradeLog.push({type:'SHORT', entry:pos.entryPrice, exit:pos.sl, roi:-15.0, time:toKST(pos.entryTime)});
          losses++; pos=null;
        }
      }
    }
  ...
  console.log(`\n=== Market Entry v6.0.0 Report (Compounded) ===`);
