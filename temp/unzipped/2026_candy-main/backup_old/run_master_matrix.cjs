...
console.log(`\n=== HYPER MATRIX PERFORMANCE: ${targetYear} ===`);
console.log(`| Retrace | Wait | SL (%) | Final Balance | Win Rate | Trades |`);
console.log(`| :--- | :--- | :--- | :--- | :--- | :--- |`);

for(let r of retraces) {
  let adjustedSL = BASE_SL - (r/100);
  for(let w of waits) {
    const res = backtest(k1m, i5m, i1h, i1d, k5m, k1h, k1d, r, w, adjustedSL);
    const wr = (res.wins/(res.wins+res.losses)*100).toFixed(1);
    console.log(`| ${r}% | ${w}m | ${(adjustedSL*100).toFixed(1)}% | $${res.balance.toFixed(2)} | ${wr}% | ${res.wins+res.losses} |`);
  }
}
