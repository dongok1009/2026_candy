...
const TARGET_NET_ROI = 0.06; // 6% Net
const SL_ROI = 0.30; // 30% SL
...
async function runV341LimitEntry() {
  console.log(`--- [v3.4.1] 백테스트: 지정가 진입 (10배, 6%/30%) ---`);
  console.log(`조건: v3.4.0 로직 기반 + 배율 10배 + 익절 6% + 손절 30%`);
  ...
  for (let i = 1; i < klines1m.length; i++) {
    ...
    if (balance <= 0) { console.log("Account Liquidated!"); break; }
    ...
    if (sig !== 'hold' && sig !== currentPosSignal) {
      ...
      for (let j = i; j < klines1m.length; j++) {
        ...
        if (ex.time % 300000 === 0 && j - i > 60) break;
      }
      ...
      if (executed) {
        ...
        for (let j = exitIdx; j < klines1m.length; j++) {
          ...
          if (sig === 'long') {
            if (ex.high >= tpPrice) { balance *= (1 + TARGET_NET_ROI); wins++; output.push(`${entryTimeKST},LONG,${limitEntryPrice},${toKSTString(ex.time)},WIN,6.00%,${balance.toFixed(2)},Executed`); i = j; break; }
            if (ex.low <= slPrice) { balance *= (1 - (SL_ROI + (MAKER_FEE_RATE + TAKER_FEE_RATE) * LEVERAGE)); losses++; output.push(`${entryTimeKST},LONG,${limitEntryPrice},${toKSTString(ex.time)},LOSS,-30.00%,${balance.toFixed(2)},Executed`); i = j; break; }
          } else {
            if (ex.low <= tpPrice) { balance *= (1 + TARGET_NET_ROI); wins++; output.push(`${entryTimeKST},SHORT,${limitEntryPrice},${toKSTString(ex.time)},WIN,6.00%,${balance.toFixed(2)},Executed`); i = j; break; }
            if (ex.high >= slPrice) { balance *= (1 - (SL_ROI + (MAKER_FEE_RATE + TAKER_FEE_RATE) * LEVERAGE)); losses++; output.push(`${entryTimeKST},SHORT,${limitEntryPrice},${toKSTString(ex.time)},LOSS,-30.00%,${balance.toFixed(2)},Executed`); i = j; break; }
          }
        }
  ...
  fs.writeFileSync('C:/dev/2026_candy/backtest_v341_results.csv', output.join('\n'));
}
