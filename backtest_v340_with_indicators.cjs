...
async function runV340WithIndicators() {
  console.log(`--- [v3.4.0] 백테스트: 지정가 진입 + 매매 지표 포함 CSV 생성 ---`);
        ...
        for (let j = exitIdx; j < klines1m.length; j++) {
          ...
          if (sig === 'long') {
            if (ex.high >= tpPrice) { 
                ...
                output.push(`${entryTimeKST},LONG,${limitEntryPrice},${toKSTString(ex.time)},WIN,3.00%,${balance.toFixed(2)},Executed,${indicatorString}`); 
                i = j; break; 
            }
            if (ex.low <= slPrice) { 
                balance *= (1 - (SL_ROI + (MAKER_FEE_RATE + TAKER_FEE_RATE) * LEVERAGE)); losses++; 
                output.push(`${entryTimeKST},LONG,${limitEntryPrice},${toKSTString(ex.time)},LOSS,-15.00%,${balance.toFixed(2)},Executed,${indicatorString}`); 
                i = j; break; 
            }
          } else {
            if (ex.low <= tpPrice) { 
                balance *= (1 + TARGET_NET_ROI); wins++; 
                output.push(`${entryTimeKST},SHORT,${limitEntryPrice},${toKSTString(ex.time)},WIN,3.00%,${balance.toFixed(2)},Executed,${indicatorString}`); 
                i = j; break; 
            }
            if (ex.high >= slPrice) { 
                balance *= (1 - (SL_ROI + (MAKER_FEE_RATE + TAKER_FEE_RATE) * LEVERAGE)); losses++; 
                output.push(`${entryTimeKST},SHORT,${limitEntryPrice},${toKSTString(ex.time)},LOSS,-15.00%,${balance.toFixed(2)},Executed,${indicatorString}`); 
                i = j; break; 
            }
          }
        }
  ...
  fs.writeFileSync('C:/dev/2026_candy/backtest_v340_results_with_indicators.csv', output.join('\n'));
}

runV340WithIndicators();
