...
// --- Config (v6.0.0 High Frequency Optimized) ---
...
async function runV600Final() {
  console.log(`--- [v6.0.0] 고빈도 최적화 버전 백테스트 (1d StochRSI 조건 제거) ---`);
  ...
  const indicators1d = { macd: calculateMACD(klines1d_all.map(k => k.close)) }; // No 1d StochRSI
  ...
  fs.writeFileSync('C:/dev/2026_candy/backtest_v600_results.csv', output.join('\n'));
