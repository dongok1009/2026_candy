...
// v4.0.0 Settings (Based on v3.1.0 but with 3m TF)
...
async function runV400Final() {
  console.log(`--- [v4.0.0] 백테스트: 3m, 1h, 1d ---`);
  console.log(`조건: 3m(Stoch), 1h(MACD+Stoch), 1d(MACD+Stoch+Filter) | No Re-entry logic`);

  const main1m_all = await fetchKlines(SYMBOL, '1m', FETCH_START_TIME, END_TIME);
  const klines3m_all = await fetchKlines(SYMBOL, '3m', FETCH_START_TIME, END_TIME);
  ...
  const output = ["EntryTime(KST),Type,Leverage,EntryPrice,ExitTime(KST),ExitPrice,Result,ROI% on Capital,Final Balance,3m_MACD,3m_Signal,3m_Hist,3m_StochK,3m_StochD,3m_Cond,1h_MACD,1h_Signal,1h_Hist,1h_StochK,1h_StochD,1h_Cond,1d_MACD,1d_Signal,1d_Hist,1d_StochK,1d_StochD,1d_Cond"];
  let m3Idx = 0;
  ...
  for (let i = 0; i < klines1m.length; i++) {
    ...
    const m3 = indicators3m.macd.m[idx3m], s3 = indicators3m.macd.s[idx3m], k3 = indicators3m.stoch.k[idx3m], d3 = indicators3m.stoch.d[idx3m];
    ...
    const h3 = m3-s3, hh = m1h-s1h, hd = m1d-s1d;

    // Logic: 3m (Stoch Only per v3.1.0 optimal logic)
    const cond3mVal = (k3 > d3) ? 'long' : (k3 < d3 ? 'short' : 'hold');
    const cond1hVal = (m1h > s1h && kh > dh) ? 'long' : (m1h < s1h && kh < dh ? 'short' : 'hold');
    const cond1dVal = (m1d > s1d && kd > dd) ? 'long' : (m1d < s1d && kd < dd ? 'short' : 'hold');

    let isL = (cond3mVal === 'long' && cond1hVal === 'long' && cond1dVal === 'long');
    let isS = (cond3mVal === 'short' && cond1hVal === 'short' && cond1dVal === 'short');
    ...
    if (signal !== currentSignal && signal !== 'hold') {
      ...
      let foundExit = false; let exitTime = 0; let finalExitPrice = 0; let resType = ""; let roiStr = "";
      ...
      if (foundExit) {
        const row = [
          ...
          m3.toFixed(2), s3.toFixed(2), h3.toFixed(2), k3.toFixed(2), d3.toFixed(2), cond3mVal,
        ...
        i--; currentSignal = signal; // No Re-entry logic
      }
  ...
  fs.writeFileSync('C:/dev/2026_candy/backtest_v400_results.csv', output.join('\n'));
}
